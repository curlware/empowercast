<?php
	// mp3info class for use in the Podops EmpowerCast
	...
	class Mp3Info {
		...
		var $m_UserAgent = 'Podops EmpowerCast';
		var $m_error = '';
		...
		{
			// Empty for now
			if( defined('EMPOWERCAST_VERSION') )
				$this->m_UserAgent = 'Podops EmpowerCast/'.EMPOWERCAST_VERSION;
			else
				$this->m_UserAgent = 'Podops EmpowerCast';
		}
		...
		*/
		function Download($url, $RedirectCount = 0)
		{
			...
			{
				$this->SetError( __('Your server must either have the php.ini setting \'allow_url_fopen\' enabled or have the PHP cURL library installed in order to continue.', 'empowercast') );
				return false;
			}
			...
			{
				$this->SetError( sprintf( __('Media URL exceeded redirect limit of %d (fopen).', 'empowercast'), $this->m_RedirectLimit) );
				return false;
			}
			...
			{
				if( empty($url) )
					$this->SetError( __('Unable to obtain host name from URL.', 'empowercast') );
				else
					$this->SetError( __('Unable to obtain  host name from the URL:', 'empowercast') .' '.$url );
				return false;
			}
			...
			if ($fp)
			{
				...
				while( !feof($fp) )
				{
					...
					else if( preg_match('/^HTTPS?\/\d\.\d (\d{3})(.*)/i', $line, $matches) )
					{
						...
						if( $ReturnCode < 200 || $ReturnCode > 206 )
						{
							fclose($fp);
							if( $ReturnCode == 404 )
								$this->SetError( __('The requested URL returned error code 404, file not found.', 'empowercast') );
							else
								$this->SetError( sprintf(__('The requested URL returned error code %d','empowercast'), $ReturnCode.$matches[2]) );
							return false;
						}
					}
					else
				...
				else // Otherwise, lets set the data and return true for part two
				{
					if( $this->m_file_size_only )
					{
						...
						{
							$this->SetError( __('Unable to obtain media size from web server.', 'empowercast') );
							return false;
						}
					}
					...
					if( function_exists('get_temp_dir') ) // If wordpress function is available, lets use it
						$TempFile = tempnam(get_temp_dir(), 'wp_empowercast');
					else // otherwise use the default path
						$TempFile = tempnam('/tmp', 'wp_empowercast');
					
					if( $TempFile === false )
					{
						$this->SetError( __('Unable to save media information to temporary directory.', 'empowercast') );
						return false;
					}
			...
			$this->SetError( __('Unable to connect to host:','empowercast') .' '.$urlParts['host']);
			return false;
		}
		...
		*/
		function DownloadCurl($url, $RedirectCount = 0)
		{
			...
			if ( $this->ini_get('safe_mode') || $this->ini_get('open_basedir') )
			{
				if( $RedirectCount > $this->m_RedirectLimit )
				{
					$this->SetError( sprintf( __('Media URL exceeded redirect limit of %d (cURL in safe mode).', 'empowercast'), $this->m_RedirectLimit) );
					return false;
				}
				$this->m_RedirectCount = $RedirectCount;
			}
			...
			if( $HttpCode < 200 || $HttpCode > 250 )
			{
				switch( $HttpCode )
				{
					...
					case 307: {
						if ( !$this->ini_get('safe_mode') && !$this->ini_get('open_basedir') )
						{
							$this->SetError( sprintf( __('Media URL exceeded redirect limit of %d (cURL).', 'empowercast'), $this->m_RedirectLimit) );
						}
						else
						{
							$redirect_url = false;
							...
							{
								$this->SetError( sprintf(__('Unable to obtain HTTP %d redirect URL.', 'empowercast'), $HttpCode) );
							}
						}
					}; break;
					case '404': {
			...
			*/
			...
			if( $this->m_file_size_only )
			{
				...
				$this->SetError( __('Unable to obtain media size from server.', 'empowercast') );
				return false;
			}
			...
			if( $TempFile === false )
			{
				$this->SetError( __('Unable to create temporary file for checking media information.', 'empowercast') );
				return false;
			}
			...
			if( !$success && curl_getinfo($curl, CURLINFO_HTTP_CODE) == 406 )
			{
				...
				{
					$this->SetError( __('Unable to download media.', 'empowercast') );
					$success = false;
				}
			}
			...
			{
				if( curl_errno($curl) )
					$this->SetError( __('Retrieving file info:', 'empowercast') .' '.  curl_error($curl) );
		...
		*/
		function GetMp3Info($File, $file_size_only = false)
		{
			...
			{
				$this->SetError( __('Error occurred downloading media file.', 'empowercast') );
				return false;
			}
			...
			{
				$this->SetError( __('Downloaded media file is empty.', 'empowercast') );
				return false;
			}
			
			//if( !preg_match('/(audio|video)/i', $this->GetContentType() ) )
			//{
			//	$this->SetError( sprintf(__('URL is reporting incorrect content type: %s', 'empowercast'), $this->GetContentType()) );
			//	return false;
			//}
			$this->_load_id3lib();
			$getID3 = new getID3;
			...
			if( $FileInfo )
			{
				...
				if( isset($FileInfo['error']) )
				{
					...
					if( !preg_match('/(audio|video)/i', $this->GetContentType() ) )
					{
						$this->SetError( sprintf(__('Media URL reporting incorrect content type: %s', 'empowercast'), $this->GetContentType()) );
						return false;
					}
				...
				if( isset($FileInfo['mpeg']['audio']) && $FileInfo['mpeg']['audio'] )
				{
					...
					{
						// Add warning here
						$this->AddWarning( sprintf(__('Sample Rate %dKhz may cause playback issues, we recommend 22Khz or 44Khz for maximum player compatibility.', 'empowercast'), $Audio['sample_rate']/1000  ) );
					}
					
					//if( stristr($Audio['channelmode'], 'stereo' ) === false )
					//{
					//	// Add warning here
					//	$this->AddWarning( sprintf(__('Channel Mode \'%s\' may cause playback issues, we recommend \'joint stereo\' for maximum player compatibility.', 'empowercast'), trim($Audio['channelmode']) ) );
					//}
				}
				*/
		...
		{
			if( defined('EMPOWERCAST_GETID3_LOADED') )
				return true; // Rock and roll
				
			if( class_exists('getID3') && !defined('EMPOWERCAST_GETID3_LOADED') )
			{
				$pre_msg = __('EmpowerCast is unable to detect media information.', 'empowercast') .'<br />';
				$getID3 = new getID3;
				if( defined('GETID3_INCLUDEPATH') ) {
					...
					if( $plugin_title )
						$this->SetError( $pre_msg. sprintf(__('Plugin \'%s\' has included a different version of the GetID3 library located in %s', 'empowercast'), $plugin_title, $path) );
					else
						$this->SetError( $pre_msg. sprintf(__('Another plugin has included a different version of the GetID3 library located in %s', 'empowercast'), $path) );
				} else {
					$this->SetError( $pre_msg. __('Another plugin has included a different version of the GetID3 library.', 'empowercast') );
				}
				return false;
			}
			...
			else if( defined('EMPOWERCAST_GETID3_LIBRARY') && is_file(EMPOWERCAST_GETID3_LIBRARY) )
				require_once(EMPOWERCAST_GETID3_LIBRARY);
			else if( defined('EMPOWERCAST_ABSPATH') )
				require_once(EMPOWERCAST_ABSPATH.'/getid3/getid3.php');
			else if( file_exists( dirname(__FILE__) .'/getid3/getid3.php' ) )
				require_once( dirname(__FILE__) .'/getid3/getid3.php');
				
			define('EMPOWERCAST_GETID3_LOADED', true);
			return true;
		}
	...
	// Example usage:
	$Mp3Info = new Mp3Info();
	$file = 'http://content.podopshost.com/geeknewscentral/GNC-2017-07-06.mp3';
