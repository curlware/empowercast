...
<div class="unlinkNetwork" id="unlinkNetwork" style="display: none;">
    <form method='POST' id="choiceForm" action="<?php echo admin_url("admin.php?page=". urlencode(empowercast_admin_get_page()) .""); ?>"> <!-- Make sure to keep back slash there for WordPress -->
		<input type="hidden" name="ppn-action" value="unset-network-id" />
        <h2 class="thickboxTitle"><?php echo esc_html(__('Are you sure you want to unlink ', 'empowercast-network')) . esc_html(get_option('empowercast_network_title') ) . '?'; ?></h2>
        <button class="warningButton" type="submit"><?php echo esc_html(__('Unlink Network', 'empowercast-network'));?></button>
    </form>
</div>
