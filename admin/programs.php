...
?>
<h2 style="font-weight: bold; font-size: 150%; margin-bottom: 1ch;"><?php echo esc_html(get_option('empowercast_network_title') ); ?>
    <form method="POST" action="#/" id="clearSiteCache" class="top-right-corner">
        <a href="#TB_inline?&width=600&height=200&inlineId=unlinkNetwork" style="color: #D21919; font-size: 70%; text-decoration: none; margin-right: 1em;" class="thickbox" title="Empowercast Network plugin"><?php echo esc_html(__('UNLINK NETWORK', 'empowercast-network'));?></a>
        <button type="button" class="cacheButton" onclick="refreshAndCallDirectAPI('Select Choice', 'clearSiteCache', 'shows')"><?php echo esc_html(__('CLEAR SITE CACHE', 'empowercast-network'));?></button>
    </form>
</h2>
        ...
        <tr class="<?php echo $class; ?>">
            ...
            <td class="<?php echo $class; ?>">
                ...
                if ($props[$i]['link'] == null){
                    ...
                    ?>
                <span style="font-weight:bold;"><a style="color: green;" href="<?php echo esc_url($props[$i]['link']);?>"><?php echo esc_html(__('View Page', 'empowercast-network'));?></a></span>
                <?php
                }
                ?>
            </td>
            ...
            <td class="<?php echo $class; ?>">
                <?php if ($props[$i]['link'] != null){ ?>
                    <a onclick="editPageForProgram(<?php echo $props[$i]['program_id']; ?>, '<?php echo $option[$key]; ?>');" href="#TB_inline?&width=500&height=300&inlineId=selectPageBox" class="thickbox" title="Empowercast Network plugin"><i class="material-icons" title="<?php echo esc_html(__('Change Page', 'empowercast-network'));?>">edit</i></a>
                    <a href="#TB_inline?&width=600&height=200&inlineId=confirmUnlink<?php echo $props[$i]['program_id']; ?>" class="thickbox" title="Empowercast Network plugin"><i class="material-icons" title="<?php echo esc_html(__('Unlink Page', 'empowercast-network'));?>">remove_from_queue</i></a>
                <?php } else { ?>
                    <a onclick="editPageForProgram(<?php echo $props[$i]['program_id']; ?>, '');" href="#TB_inline?&width=500&height=300&inlineId=selectPageBox" class="thickbox" title="Empowercast Network plugin"><i class="material-icons" title="<?php echo esc_html(__('Link to Existing Page', 'empowercast-network'));?>">edit</i></a>
                    <a href="" onclick="createPage(<?php echo esc_html($props[$i]['program_id']); ?>, 'Program','createForm', '<?php echo esc_html($props[$i]['program_title']); ?>'); return false;"><i class="material-icons" title="<?php echo esc_html(__('Create New Page', 'empowercast-network'));?>">add_to_queue</i></a>
                <?php } ?>
                <a onclick="jQuery('#add-program-to-group').val(<?php echo $props[$i]['program_id']; ?>);" href="#TB_inline?&width=500&height=200&inlineId=addToGroup" class="thickbox" title="Empowercast Network plugin"><i class="material-icons" title="<?php echo esc_html(__('Add to Group', 'empowercast-network'));?>">list</i></a>
                <a href="#TB_inline?&width=500&height=200&inlineId=confirmRemoval<?php echo $props[$i]['program_id']; ?>" class="thickbox" title="Empowercast Network plugin"><i class="material-icons" title="<?php echo esc_html(__('Delete Program', 'empowercast-network'));?>">delete</i></a>
            </td>
        </tr>
        <?php
    }
...
<div class="selectPageBox" id="selectPageBox" style="display: none">
    ...
    <button type="submit" class="ppn-back-button" onclick="directStatus('Select Choice', 'changeForm', true, 'shows')"><?php echo esc_html(__('Save', 'empowercast-network'));?></button>
...
<div class="confirmUnlink" id="confirmUnlink<?php echo $props[$i]['program_id']; ?>" style="display: none">
    ...
    <p style="color: black; font-weight: bold"><?php echo esc_html(__('Are you sure you want to unlink the current page off the program ' . esc_html($props[$i]['program_title']) . '?', 'empowercast-network'));?></p><br>
    <button type="submit" class="warningButton" onclick="confirmUnlink('createForm<?php echo $props[$i]['program_id']; ?>');directStatus('Select Choice', 'createForm<?php echo $props[$i]['program_id']; ?>', false, 'shows')"><?php echo esc_html(__('Unlink page', 'empowercast-network'));?></button>
    <p class="ppn-back-button" onclick="tb_remove();"><?php echo esc_html(__('Cancel', 'empowercast-network'));?></p>
</div>
<?php } ?>
...
<div class="confirmRemoval" id="confirmRemoval<?php echo $props[$i]['program_id']; ?>" style="display: none;">
    ...
    <button type="submit" class="warningButton" onclick="confirmRemovalOfProgram(<?php echo esc_html($props[$i]['program_id']); ?>);"><?php echo esc_html(__('Remove program', 'empowercast-network')); ?>
</div>
<?php
}
