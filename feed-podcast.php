 ...
 */
	...
	function empowercast_get_the_content_feed($feed_type, $no_filters = false)
	{
		if( $no_filters == false )
	...
	function empowercast_get_the_excerpt_rss($no_filters = true)
	{
		if( $no_filters == false ) {
	...
	function empowercast_the_generator() {
	...
	if( !empty($GeneralSettings['new_episode_box_feature_in_itunes']) ) {
		...
		if( !empty($iTunesFeatured[ $feed_slug ]) )
		{
			if( get_post_type() == 'post' )
			{
				...
				$GLOBALS['empowercast_feed']['itunes_feature'] = true; // So any custom order value is not used when looping through the feeds.
...
<channel>
	...
	<?php if( !empty($FeedActionHook) ) { echo '<generator>Podops EmpowerCast/' . EMPOWERCAST_VERSION . '</generator>'. PHP_EOL; } ?>
...
<?php
    ...
    while( have_posts()) :
        the_post();
        $EpisodeData = empowercast_get_enclosure_data(get_the_ID());
...
<?php
		if( empty($GLOBALS['empowercast_feed']['feed_maximizer_on']) ) // If feed maximizer off
		{
...
<?php } // end empowercast feed comments  

	if( empty($GeneralSettings['feed_accel']) ) {
		the_category_rss('rss2');
	}
				if (get_option('rss_use_excerpt')) { ?>
		<description><?php echo empowercast_format_itunes_value( empowercast_get_the_excerpt_rss( !empty($GeneralSettings['feed_action_hook']) ), 'description' ); ?></description>
<?php } else { // else no rss_use_excerpt ?>
		<description><?php echo empowercast_format_itunes_value( empowercast_get_the_excerpt_rss( !empty($GeneralSettings['feed_action_hook']) ), 'description' ); ?></description>
<?php $content = empowercast_get_the_content_feed('rss2', !empty($GeneralSettings['feed_action_hook']) ); ?>
<?php if ( strlen( $content ) > 0 ) { ?>
		<content:encoded><![CDATA[<?php echo $content; ?>]]></content:encoded>
<?php } else { //  else strlen( $post->post_content ) <= 0 ?>
		<content:encoded><![CDATA[<?php echo empowercast_get_the_excerpt_rss( !empty($GeneralSettings['feed_action_hook']) ); ?>]]></content:encoded>
...
<?php rss_enclosure(); ?>
	...
	if( empty($GLOBALS['empowercast_feed']['feed_maximizer_on']) && $ItemCount >= 10 && !empty($GLOBALS['empowercast_feed']['maximize_feed']) )
	{
		$GLOBALS['empowercast_feed']['feed_maximizer_on'] = true; // All future items will be minimized in order to maximize episode count
	}
	?>
</item>
<?php
...
?>
	<item>
		...
		<pubDate><?php echo mysql2date('D, d M Y H:i:s +0000', get_post_time('Y-m-d H:i:s', true), false); ?></pubDate>
		<guid isPermaLink="false"><?php the_guid(); ?></guid>
		<description><?php echo empowercast_format_itunes_value( empowercast_get_the_excerpt_rss( !empty($GeneralSettings['feed_accel']) ), 'description' ); ?></description>
<?php rss_enclosure(); ?>
...
if( defined('EMPOWERCAST_DEBUG_QUERIES') ) {
