...
class EmpowerCastAuth {
    ...
    var $apiUrl = array('https://api.podopshost.com/');
    var $apiUrlIndex = 0;

    function __construct() {

        if( defined('EMPOWERCAST_PODOPS_API_URL') ) {
            if( strstr(EMPOWERCAST_PODOPS_API_URL, 'http://api.podopshost.com') == false ) // If not the default
            {
                $this->apiUrl = explode(';', EMPOWERCAST_PODOPS_API_URL);
            }
            else
            {
                $this->apiUrl[] = 'https://api.podopshost.com/'; // Use secure URL first when possible
                $this->apiUrl[] = 'http://api.podops.net/';
                $this->apiUrl[] = 'http://api.podopshost.com/';
            }
        }
    }
    ...
    private function _makeApiCallCurl($url, $post = false, $clientCredsBase64 = false, $bearerValue = '', $custom_request = false, $timeout = 15, $decode_json = true, $encode_json = false ) {
        ...
        else if( $http_code > 399 ) // HTTP level error, lets record it and see if the response is what we want to use...
        {
            ...
            switch( $http_code )
            {
                case 400: $this->error .= ' '. __("Bad Request", 'empowercast'); break;
                case 401: $this->error .= ' '. __("Unauthorized (Check that your username and password are correct)", 'empowercast'); break;
                case 402: $this->error .= ' '. __("Payment Required", 'empowercast'); break;
                case 403: $this->error .= ' '. __("Forbidden", 'empowercast'); break;
                case 404: $this->error .= ' '. __("Not Found", 'empowercast'); break;
            }
        }
