...
function empowercast_admin_capabilities()
{
	global $wp_roles;
	...
	$capnames = apply_filters( 'empowercast_admin_capabilities', array_unique($capnames) );
...
// empowercastadmin_editfeed.php
function empowercast_admin_editfeed($type='', $type_value = '', $feed_slug = false)
{
	...
	$FeedTitle = __('Feed Settings', 'empowercast');
	$NewPostQueryString = '';
	switch( $type )
	{
		case 'channel': {
			...
			$FeedSettings = empowercast_get_settings('empowercast_feed_'.$feed_slug);
			...
			$FeedSettings = empowercast_default_settings($FeedSettings, 'editfeed_custom');
			
			if( !isset($General['custom_feeds'][$feed_slug]) )
				$General['custom_feeds'][$feed_slug] = __('Podcast (default)', 'empowercast');
			...
			echo '<input type="hidden" name="action" value="empowercast-save-channel" />';
			
		}; break;
		case 'category': {
			$cat_ID = $type_value; 
			$FeedAttribs['category_id'] = $type_value;
			$FeedSettings = empowercast_get_settings('empowercast_cat_feed_'.$cat_ID);
			$FeedSettings = empowercast_default_settings($FeedSettings, 'editfeed_custom');
			
			$category = get_category_to_edit($cat_ID);
			$FeedTitle = sprintf( __('Podcast Settings for Category: %s', 'empowercast'), htmlspecialchars($category->name) );
			echo sprintf('<input type="hidden" name="cat" value="%s" />', $cat_ID);
			echo '<input type="hidden" name="action" value="empowercast-save-category" />';
			
		}; break;
		case 'ttid': {
			$term_taxonomy_id = $type_value;
			$FeedAttribs['term_taxonomy_id'] = $type_value;
			$FeedSettings = empowercast_get_settings('empowercast_taxonomy_'.$term_taxonomy_id);
			$FeedSettings = empowercast_default_settings($FeedSettings, 'editfeed_custom');
			...
			if( !empty( $term_info[0]['term_id']) ) {
				...
				$FeedTitle = sprintf( __('Podcast Settings for Taxonomy Term: %s', 'empowercast'), htmlspecialchars($term_object->name));
                echo sprintf('<input type="hidden" name="term" value="%s" />', $term_ID);
			}
			else
			{
				$FeedTitle = sprintf( __('Podcast Settings for Taxonomy Term: %s', 'empowercast'), 'Term ID '.htmlspecialchars($term_taxonomy_id));
			}
			echo sprintf('<input type="hidden" name="ttid" value="%s" />', $term_taxonomy_id);
			echo '<input type="hidden" name="action" value="empowercast-save-ttid" />';
		...
		case 'post_type': {
			...
			$FeedSettingsArray = empowercast_get_settings('empowercast_posttype_'.$FeedAttribs['post_type']);
			if( !is_array($FeedSettingsArray[ $feed_slug ]) )
				$FeedSettingsArray[ $feed_slug ] = array();
			$FeedSettings = empowercast_default_settings($FeedSettingsArray[ $feed_slug ], 'editfeed_custom');
			$NewPostQueryString = '?post_type=' . $type_value;
			//$category = get_category_to_edit($cat_ID);
			$PostTypeTitle = $FeedAttribs['post_type']; // TODO: Get readable title of post type
			$FeedTitle = sprintf( __('Podcast Settings for Post Type %s with slug %s', 'empowercast'), htmlspecialchars($PostTypeTitle) , htmlspecialchars($feed_slug));
			echo sprintf('<input type="hidden" name="podcast_post_type" value="%s" />', $FeedAttribs['post_type']);
			echo sprintf('<input type="hidden" name="feed_slug" value="%s" />', $feed_slug);
			echo '<input type="hidden" name="action" value="empowercast-save-post_type" />';
			
		}; break;
		default: {
			$FeedSettings = empowercast_get_settings('empowercast_feed');
			$FeedSettings = empowercast_default_settings($FeedSettings, 'editfeed');
			echo '<input type="hidden" name="action" value="empowercast-save-settings" />';
		}; break;
	}
...
?>
    <div id="empowercast_settings_page" class="empowercast_tabbed_content">
        <div class="pp-tab">
            ...
            <!-- <button class="tablinks" onclick="openTab(event, 'settings-analytics')"><?php echo htmlspecialchars(__('Analytics', 'empowercast')); ?></button> -->
            <button id="other-tab" class="tablinks" onclick="empowercast_openTab(event, 'settings-other')"><?php echo htmlspecialchars(__('Other', 'empowercast')); ?></button>
        ...
        <div id="settings-welcome" class="pp-tabcontent active">
            ...
            <div id="welcome-all" class="pp-sidenav-tab active">
                <?php empowercastadmin_welcome($General, $FeedSettings, $NewPostQueryString); ?>
            </div>
        </div>
        ...
        <div id="settings-feeds" class="pp-tabcontent has-sidenav">
            <div class="pp-sidenav-toggle-container">
                ...
                <div class="pp-sidenav">
                    ...
                    <button id="feeds-default-open" class="pp-sidenav-tablinks active" id="feeds-settings-tab" onclick="sideNav(event, 'feeds-settings')"><img class="pp-nav-icon" alt="" src="<?php echo empowercast_get_root_url(); ?>images/settings_nav_icons/option_bar_settings_gray.svg"><?php echo htmlspecialchars(__('Feed Settings', 'empowercast')); ?></button>
            ...
            <div id="feeds-settings" class="pp-sidenav-tab active">
                <?php empowercastadmin_edit_feed_settings($FeedSettings, $General, $FeedAttribs);
                empowercast_settings_tab_footer(); ?>
            </div>
            <div id="feeds-artwork" class="pp-sidenav-tab">
                <?php empowercastadmin_edit_artwork($FeedSettings, $General);
                empowercast_settings_tab_footer();  ?>
            </div>
            <div id="feeds-basic" class="pp-sidenav-tab">
                <?php empowercastadmin_edit_funding($FeedSettings, $feed_slug);
                empowercast_settings_tab_footer();  ?>
            </div>
            <div id="feeds-rating" class="pp-sidenav-tab">
                <?php empowercastadmin_edit_tv($FeedSettings, $feed_slug);
                empowercast_settings_tab_footer();  ?>
            </div>
            <div id="feeds-apple" class="pp-sidenav-tab">
                <?php empowercastadmin_edit_itunes_feed($FeedSettings, $General, $FeedAttribs);
                empowercast_settings_tab_footer();  ?>
            </div>
        </div>
        ...
        <div id="settings-website" class="pp-tabcontent">
            ...
            <div id="website-settings" class="pp-sidenav-tab active">
                <?php empowercastadmin_website_settings_custom_feed($General, $FeedSettings, $FeedAttribs);
                empowercast_settings_tab_footer();  ?>
            </div>
            <div id="website-shortcodes" class="pp-sidenav-tab">
                <?php empowercast_shortcode_settings($General, $FeedAttribs);
                empowercast_settings_tab_footer();  ?>
            </div>
        </div>
        ...
        <div id="settings-other" class="pp-tabcontent">
            <div class="pp-sidenav-toggle-container">
                <div id="other-toggle-sidenav" class="toggle-sidenav" title="Podops Services" onclick="empowercast_displaySideNav(this);">&lt;</div>
                <div class="pp-sidenav">
                    <?php
                    empowercastadmin_edit_podops_services($General);
                    ?>
            ...
            <button style="display: none;" id="other-default-open" class="pp-sidenav-tablinks active" onclick="sideNav(event, 'other-all')"><img class="pp-nav-icon" style="width: 22px;" alt="" src="<?php echo empowercast_get_root_url(); ?>images/settings_nav_icons/rss-symbol.svg"><?php echo htmlspecialchars(__('Hidden button', 'empowercast')); ?></button>
            <div id="other-all" class="pp-sidenav-tab active">
                <?php
                empowercastadmin_settings_tab_other($General, $FeedSettings, $feed_slug, $cat_ID, $FeedAttribs);
                empowercast_settings_tab_footer();
                ?>
            </div>
        </div>
        ...
        <div id="settings-live-item" class="pp-tabcontent">
            <div class="pp-sidenav-toggle-container">
                ...
                <?php
                empowercastadmin_live_item_options($FeedSettings);
                ?>
        </div>
...
function empowercastadmin_edit_podcast_channel($FeedSettings, $General)
{
	// TODO
?>
<input type="hidden" name="action" value="empowercast-save-customfeed" />
<p style="margin-bottom: 0;">
	<?php echo __('Configure your custom podcast feed.', 'empowercast'); ?>
</p>
<?php
}

function empowercastadmin_edit_category_feed($FeedSettings, $General)
{
?>
<input type="hidden" name="action" value="empowercast-save-categoryfeedsettings" />
<p style="margin-bottom: 0;">
	<?php echo __('Configure your category feed to support podcasting.', 'empowercast'); ?>
</p>
<?php
}

function empowercastadmin_edit_feed_general($FeedSettings, $General, $FeedAttribs)
{
    require_once('empowercastadmin-tags.php');
	$warning = '';
	$episode_count = empowercast_get_episode_count('podcast');
	if( $episode_count == 0 )
	{
		$warning = __('WARNING: You must create at least one podcast episode for your podcast feed to be valid.', 'empowercast');
	}
...
?>
<h1 class="pp-heading"><?php echo __('Podcast Feeds', 'empowercast'); ?></h1>
<p class="pp-settings-text"><?php echo __('Your podcast RSS feed: ', 'empowercast'); ?>
    <a href="<?php echo esc_attr($feed_link); ?>"> <?php echo esc_attr($feed_link); ?> </a>
</p>

<div class="pp-settings-section">
    <h2><?php echo __('Enhance Feeds', 'empowercast'); ?></h2>
    <ul>
        <li>
            ...
            <div class="pp-settings-subsection">
                <p class="pp-main"><?php echo __('Enhance All Feeds', 'empowercast'); ?> (<?php echo __('Recommended', 'empowercast'); ?>)</p>
                <p class="pp-sub"><?php echo __('Adds podcasting support to all feeds', 'empowercast'); ?></p>
            </div>
        </li>
        <li>
            <input class="pp-settings-radio" type="radio" name="Feed[apply_to]" value="2" <?php if( $FeedSettings['apply_to'] == 2 ) echo 'checked'; ?> />
            <div class="pp-settings-subsection">
                <p class="pp-main"><?php echo __('Enhance Main Feed Only', 'empowercast'); ?></p>
                <p class="pp-sub"><?php echo __('Adds podcasting support to your main feed only', 'empowercast'); ?></p>
            </div>
        </li>
        <li>
            <input class="pp-settings-radio" type="radio" name="Feed[apply_to]" value="0" <?php if( $FeedSettings['apply_to'] == 0 ) echo 'checked'; ?> />
            <div class="pp-settings-subsection">
                <p class="pp-main"><?php echo __('Do Not Enhance Feeds', 'empowercast'); ?></p>
                <p class="pp-sub"><?php echo __('Feed Settings below will only apply to your podcast channel feeds', 'empowercast'); ?></p>
            </div>
        </li>
    </ul>
</div>
<div class="pp-settings-section">
    <h2><?php echo __('Podcast Feeds', 'empowercast'); ?></h2>
<?php if( $warning ) { ?>
<span class="empowercast-error" style="background-color: #FFEBE8; border-color: #CC0000; padding: 6px 10px;"><?php echo $warning; ?></span>
<?php } ?>
<?php
	
	//$General = get_option('empowercast_general');
	$Feeds = array('podcast'=> __('Special Podcast only Feed', 'empowercast') );
	...
	foreach( $Feeds as $feed_slug=> $feed_title )
	{
		...
		$edit_link = admin_url( 'admin.php?page=empowercast/empowercastadmin_customfeeds.php&amp;action=empowercast-editfeed&amp;feed_slug=') . $feed_slug;
?>
	...
	<?php if( defined('EMPOWERCAST_FEEDVALIDATOR_URL') ) { ?>
	- <a href="<?php echo EMPOWERCAST_FEEDVALIDATOR_URL. urlencode(get_feed_link($feed_slug)); ?>" target="_blank"><?php echo __('Validate Feed', 'empowercast'); ?></a>
	<?php } ?>
	<?php if( false && $feed_slug != 'podcast' ) { ?>
	- <a href="<?php echo $edit_link; ?>" title="<?php echo __('Edit Podcast Channel', 'empowercast'); ?>"><?php echo __('Edit', 'empowercast'); ?></a>
	<?php } ?>
        </p>
<?php } ?>
<p class="pp-settings-text"><?php echo __('These are podcast only feeds suitable for submission podcast directories such as Apple Podcasts.', 'empowercast'); ?></p>
<p style="margin-bottom: 3ch;" class="description pp-settings-text"><?php echo __('Note: We do not recommend submitting your main site feed to podcast directories such as iTunes. iTunes and many other podcast directories work best with feeds that do not have regular blog posts mixed in.', 'empowercast');  ?></p>
...
<div>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-settings-text"><?php echo __('Do not allow other plugins to modify podcast feeds.', 'empowercast'); ?></p>
    </div>
</div>
<div>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-settings-text"><?php echo __('Accelerate feed', 'empowercast'); ?></p>
    </div>
</div>
    ...
    <div class="pp-settings-section">
        <h2><?php echo __('Suppress Unused Item Tags', 'empowercast'); ?></h2>
        ...
        <div class="pp-settings-subsection">
            <p class="pp-main"><?php echo __('Exclude the itunes:* tags for subtitle, summary, author, and isClosedCaptioned from all items in podcast feeds.', 'empowercast'); ?>
            <p class="pp-label-bottom"><?php echo __('These tags have all been removed from Apple\'s documentation.', 'empowercast'); ?></p></p>
        </div>
    </div>

<div class="pp-settings-section">
    <h2><?php echo __('Feed Discovery', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-main"><?php echo __('Include podcast feed links in HTML headers.', 'empowercast'); ?>
        <p class="pp-label-bottom"><?php echo __('Adds "feed discovery" links to your web site\'s headers allowing web browsers and feed readers to auto-detect your podcast feeds.', 'empowercast'); ?></p></p>
    </div>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('RSS2 Image', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-main"><?php echo __('Include RSS Image in feeds.', 'empowercast'); ?></label></p>
    </div>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Emoji', 'empowercast'); ?></h2>
<?php
...
<p class="pp-settings-text" style="font-weight: bold; color: #CC0000;">
	<?php 
		echo __('Emoji may not be supported with your WordPress installation. Please upgrade your database to support utf8mb4 available in WordPress 4.2 and newer.', 'empowercast');
		?>
	</p>
<?php }  ?>
...
<input class="pp-settings-checkbox" type="checkbox" name="General[rss_emoji]" value="1" <?php if( !empty($General['rss_emoji']) ) echo 'checked '; ?>/>
    <div class="pp-settings-subsection">
        <p class="pp-main"><?php echo __('Include Emoji in feeds.', 'empowercast'); ?></p>
        <p class="pp-label-bottom"><a href="https://podopshost.com/support/empowercast-documentation/feeds/#emoji" target="_blank"><?php echo __('Learn more', 'empowercast'); ?></a></p>
    </div>
</div>
    <?php empowercast_admin_tags(); ?>
<?php
}

function empowercastadmin_edit_feed_settings($FeedSettings, $General, $FeedAttribs = array() )
{
	$SupportUploads = empowercastadmin_support_uploads();
...
?>
<h1 class="pp-heading"><?php echo __('Feed Settings', 'empowercast'); ?></h1>
<p class="pp-settings-text"><?php echo __('Your podcast RSS feed: ', 'empowercast'); ?>
    <a href="<?php echo esc_attr($feed_link); ?>"> <?php echo esc_attr($feed_link); ?> </a>
</p>

<div class="pp-settings-section">
    <h2><?php echo __('Feed Title', 'empowercast'); ?></h2>
    <label for="Feed[title]" class="pp-settings-label"><?php echo __('Show Title', 'empowercast'); ?></label>
...
<?php if( $cat_ID ) { ?>
<?php echo __('Leave blank to use default category title', 'empowercast'); ?>
<?php } else { ?>
<?php echo __('Leave blank to use blog title for the show', 'empowercast'); ?>
<?php } ?>
    </label>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Feed Description', 'empowercast'); ?></h2>
    <label for="Feed[description]" class="pp-settings-label"><?php echo __('Show Description', 'empowercast'); ?></label>
...
<?php if( $cat_ID ) { ?>
<?php echo __('Leave blank to use category description', 'empowercast'); ?>
<?php } else { ?>
<?php echo __('Leave blank to use blog description', 'empowercast'); ?>
<?php } ?>
    </label>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Feed Landing Page URL', 'empowercast'); ?></h2>
...
<?php if( $cat_ID ) { ?>
<?php echo __('Leave blank to use category page', 'empowercast'); ?>
<?php } else { ?>
<?php echo __('Leave blank to use home page', 'empowercast'); ?>
<?php } ?>
<?php if( $cat_ID ) { ?>
    <p class="description"><?php echo __('Category page URL', 'empowercast'); ?>: <?php echo get_category_link($cat_ID); ?></p>
<?php } else { ?>
...
<div class="pp-settings-section">
    <h2><?php echo __('PodcastMirror Feed URL', 'empowercast'); ?><br /></h2>
    <label for="Feed[feed_redirect_url]" class="pp-settings-label"><?php echo __('URL', 'empowercast'); ?></label>
    ...
    <label for="Feed[feed_redirect_url]" class="pp-settings-label-under">
        <?php echo __('Leave blank to use built-in feed', 'empowercast'); ?>
    </label>
    <span class="pp-right-label" style="float: right;"><a href="https://podcastmirror.com" target="_blank"><?php echo __('Learn more', 'empowercast'); ?></a></span>
    <p style="margin-top: 1em;" class="pp-settings-text"><?php echo __('Use this option to mirror your podcast feed to provide fast, scalable subscriptions to your show. This option is not required. Service is FeedBurner compatible.', 'empowercast'); ?></p>
<?php
$link = $feed_link;
if( strstr($link, '?') )
	$link .= "&redirect=no";
	...
	$link .= "?redirect=no";
?>
    <p class="pp-settings-text"><?php echo __('Bypass Redirect URL', 'empowercast'); ?>: <a href="<?php echo $link; ?>" target="_blank"><?php echo $link; ?></a></p>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Show the most recent', 'empowercast'); ?></h2>
    <p class="pp-settings-text"><?php echo __('Please enable the Feed Episode Maximizer option to optimize your feed for more than 10 episodes.', 'empowercast'); ?></p>
    <label for="Feed[posts_per_rss]" class="pp-settings-label"><?php echo __('Show the most recent', 'empowercast'); ?></label>
    ...
    <label for="Feed[posts_per_rss]" class="pp-settings-label-under">
        <?php echo sprintf(__('episodes / posts per feed (site default: %d)', 'empowercast'), get_option('posts_per_rss')); ?>
    </label>
</div>

    <div class="pp-settings-section">
        <h2><?php echo __('Feed Lock', 'empowercast'); ?><br /></h2>
        <input type="checkbox" onclick="empowercast_toggle_lock_section(event)" class="pp-settings-checkbox" name="Feed[pp_enable_feed_lock]" value="1" <?php echo ( !empty($FeedSettings['pp_enable_feed_lock']) || empty($FeedSettings['email']) ?'checked ':''); ?>/>
        <div class="pp-settings-subsection" style="border: none;">
            <p class="pp-sub"><?php echo __('Enable Feed Lock', 'empowercast'); ?></p>
        </div>
        <div id="pp-feed-lock-section"  style="display: <?php echo empty($FeedSettings['pp_enable_feed_lock']) ? "none" : "block"; ?>">
            <div>
                <input class="pp-settings-radio-small" type="radio" style="margin: 2ch 8px 0 1em;vertical-align: top;" class="empowercast_lock_option" id="empowercast_lock_option_1" name="Feed[unlock_podcast]" value="0" <?php if( empty($FeedSettings['unlock_podcast']) ) echo 'checked'; ?> />
                <div class="pp-settings-subsection-no-border" style="padding-bottom: 0;">
                    <p class="pp-settings-text" style="margin: 0;"><?php echo __('Yes - Podcast cannot be imported to a new platform.', 'empowercast'); ?></p>
                    <p class="pp-sub" style="font-size: 14px"><?php echo __('Default', 'empowercast'); ?></p>
                </div>
            </div>
            <div>
                <input class="pp-settings-radio-small" type="radio" style="margin: 2ch 8px 0 1em;vertical-align: top;" class="empowercast_lock_option" id="empowercast_lock_option_2" name="Feed[unlock_podcast]" value="1" <?php if( isset($FeedSettings['unlock_podcast']) && $FeedSettings['unlock_podcast'] == 1 ) echo 'checked'; ?> />
                <div class="pp-settings-subsection-no-border">
                    <p class="pp-settings-text" style="margin: 0;"><?php echo __('No - Podcast can be imported to a new platform.', 'empowercast'); ?></p>
                </div>
            </div>
        </div>
    </div>
...
?>
<div class="pp-settings-section">
    <h2><?php echo __('Feed Episode Maximizer', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-main"><?php echo __('Maximize the number of episodes while maintaining an optimal feed size.', 'empowercast'); ?></p>
        <p class="pp-settings-text"><a href="https://podopshost.com/support/empowercast-documentation/feeds/#maximizer" target="_blank"><?php echo __('Learn more', 'empowercast'); ?></a></p>
    </div>
</div>
...
<div class="pp-settings-section">
    <h2><?php echo __('Feed Language', 'empowercast'); ?></h2>
    <select class="pp-settings-select" name="Feed[rss_language]">
<?php
$Languages = empowercast_languages();

echo '<option value="">'. __('Blog Default Language', 'empowercast') .'</option>';
...
{
?>
 <?php echo __('Blog Default', 'empowercast'); ?>: <?php echo $Languages[ $rss_language ]; ?>
 <?php } else {  ?>
<?php echo __('Blog Default', 'empowercast'); ?>: <?php echo $rss_language; ?>
 <?php } ?>
    </label>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Copyright', 'empowercast'); ?></h2>
    <label for="Feed[copyright]" class="pp-settings-label"><?php echo __('Copyright text', 'empowercast'); ?></label>
...
<div class="pp-settings-section">
    <h2><?php echo __('Author Email', 'empowercast'); ?></h2>
    <input type="text" class="pp-settings-text-input" name="Feed[email]" value="<?php echo esc_attr($FeedSettings['email']); ?>" maxlength="255" />
    <label for="Feed[email]" class="pp-settings-label-under"><?php echo __('Apple will email this address when your podcast is accepted into the Apple Podcast Directory.', 'empowercast'); ?></label>
...
<div class="pp-settings-section">
    <div class="row">
        <h2 style="margin: 0;">
            <?php echo __('Shows & Content You Love', 'empowercast'); ?>
        </h2>
        <div class="pp-tooltip-right" style="margin-left: 5px; height: 16px; width: 16px;">i
            <span class="text-pp-tooltip" style="top: -50%; min-width: 200px;"><?php echo esc_html(__('You can now recommend podcasts you love and or specific episodes of content that will aggregated by podcast apps and presented to your listeners in apps found at podcastapps.com', 'empowercast')); ?></span>
        </div>
    </div>
    <?php if (!empty(get_option('empowercast_creds'))) { ?>
    <div class="col mt-3" id="remote-item-add-block">
        <div class="row">
            <div class="form-check" style="margin-right: 20px;">
                ...
                <label class="form-check-label" for="daily" style="color: black; font-size: 1rem;">
                    <?php echo __("Add to PodRoll", "empowercast"); ?>
                    <div class="pp-tooltip-right" style="margin-left: 5px; height: 16px; width: 16px;">i
                        <span class="text-pp-tooltip" style="top: -50%; min-width: 200px;"><?php echo esc_html(__('Add shows that you would like to recommend to your listeners.', 'empowercast')); ?></span>
                    </div>
                </label>
            </div>
            <div class="form-check">
                ...
                <label class="form-check-label" for="weekly" style="color: black; font-size: 1rem;">
                    <?php echo __("Add to RSS Feed", "empowercast"); ?>
                    <div class="pp-tooltip-right" style="margin-left: 5px; height: 16px; width: 16px;">i
                        <span class="text-pp-tooltip" style="top: -50%; min-width: 200px;"><?php echo esc_html(__('Have an external podcast episode or resource you would like to highlight added to your RSS feed.', 'empowercast')); ?></span>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div class="row mt-3" id="add-episode-container" style="display: none; justify-content: flex-end;">
            <a id="add-remote-item" style="color: #1976d2; cursor: pointer; font-weight: bold;">
                <?php echo __("Add Content", "empowercast"); ?>
            </a>
        </div>
    </div>
    <?php
    ...
    <div class="row mt-2">
        <div class="col-md-6">
            <h3 style="font-weight: bold;">
                <?php echo __("PodRoll Shows", "empowercast"); ?>
            </h3>
                    ...
                    <div id="remote-item-<?php echo $item['feed_guid'];?>">
                        <div class="row pl-3 pr-2" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ddd;">
                            <h4 style="margin: 0; padding-left: 5%;" class="list-result">
                                <?php echo __(htmlspecialchars($item['item_title']) ?? 'RemoteItem', "empowercast"); ?>
                            </h4>
        ...
        <div class="col-md-6">
            <h3 style="font-weight: bold;">RSS Feed
                <?php echo __("RSS Feed", "empowercast"); ?>
            </h3>
            <div class="col" style="border-radius: 5px; border: 1px solid #E2E2E2;" id="feed-list-col">
                <?php foreach($existingFeedItems as $item) { ?>
                    <div id="remote-item-<?php echo $item['feed_guid'];?>">
                        <div class="row pl-3 pr-2" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ddd;">
                            <h4 style="margin: 0; padding-left: 5%;" class="list-result">
                                <?php echo __(htmlspecialchars($item['item_title']), "empowercast"); ?>
                            </h4>
    ...
    <div class="col-mt-3">
        ...
        ?>
        <p><?php echo esc_html(__('A Podops account is required to use this feature.', 'empowercast')); ?> <a target="_blank" href="<?php echo admin_url("admin.php?page=empowercastadmin_onboarding.php&step=wantStats"); ?>"><?php echo esc_html(__('Create a free account', 'empowercast')); ?></a> or link your existing account <a target="_blank" href="<?php echo esc_attr(add_query_arg( '_wpnonce', $pp_nonce, admin_url("admin.php?page=empowercastadmin_onboarding.php&step=podopsSignin&from=empowercastadmin_basic"))); ?>"><?php echo esc_html(__('here', 'empowercast')); ?></a>.</p>
    </div>
...
<div class="pp-settings-section">
    <h2>
        <?php echo __('Medium Tag', 'empowercast'); ?>
        <div class="pp-tooltip-right" style="height: 16px; width: 16px; font-size: 80%;">i
            <span class="text-pp-tooltip" style="top: -50%; min-width: 200px;"><?php echo esc_html(__('This setting tells an application what the content contained within the feed IS, as opposed to what the content is ABOUT in the case of a category.', 'empowercast')); ?></span>
        </div>
    </h2>
...
<div class="pp-settings-section">
    <h2><?php echo __('GUID', 'empowercast'); ?></h2>
    <input type="checkbox" onclick="empowercast_toggle_guid_section(event)" class="pp-settings-checkbox" name="Feed[guid_override_check]" value="1" <?php echo ( !empty($FeedSettings['guid_override_check']) ?'checked ':''); ?>/>
    <div class="pp-settings-subsection" style="border: none;">
        <p class="pp-sub"><?php echo __('Enable GUID Override', 'empowercast'); ?></p>
    </div>
    <div id="pp-guid-override-section"  style="display: <?php echo empty($FeedSettings['guid_override_check']) ? "none" : "block"; ?>">
        <label for="Feed[guid_override]" class="pp-settings-label"><?php echo __('Override GUID', 'empowercast'); ?></label>
        ...
        <label for="Feed[guid_override]" class="pp-settings-label-under">
            <?php echo __('You should only override the default EmpowerCast GUID if you have an existing GUID for your show.', 'empowercast'); ?>
        </label>
    </div>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Caching Debug Comments', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-main">
	    <?php echo __('Allow WP Super Cache or W3 Total Cache to add HTML Comments to the bottom of your feeds', 'empowercast'); ?>
	    (<?php echo __('Recommended unchecked', 'empowercast'); ?>)
        </p>
    </div>
    <p class="pp-label-bottom" style="margin-top: 2ch;"><?php echo __('iTunes is known to have issues with feeds that have HTML comments at the bottom.', 'empowercast'); ?>
    <?php echo __('NOTE: This setting should only be enabled for debugging purposes.', 'empowercast'); ?></p>
...
function empowercastadmin_settings_tab_other($General, $FeedSettings, $feed_slug, $cat_ID = false,  $FeedAttribs = array() ) {

	require_once( dirname(__FILE__).'/views/settings_tab_other.php' );
}

function empowercastadmin_settings_tab_appearance($General, $FeedSettings, $FeedAttribs = array()) {
	require_once( dirname(__FILE__).'/views/settings_tab_appearance.php' );
}

function empowercastadmin_edit_appearance_feed($General,  $FeedSettings, $feed_slug)
{
	// Appearance Settings
?>
<h3><?php echo __('Website Settings', 'empowercast'); ?></h3>
<table class="form-table">
<tr valign="top">
<th scope="row">
<?php echo __('Disable Player', 'empowercast'); ?>
</th>
<td>
	<input name="DisablePlayerFor" type="checkbox" <?php if( isset($General['disable_player'][$feed_slug]) ) echo 'checked '; ?> value="1" /> <?php echo __('Do not display web player or links for this podcast.', 'empowercast'); ?>
...
function empowercastadmin_edit_itunes_feed($FeedSettings, $General, $FeedAttribs = array() )
{
...
<h1 class="pp-heading"><?php echo __('Apple Settings', 'empowercast'); ?></h1>
<p class="pp-settings-text">
    <?php echo __('The following settings will affect the display of your podcast\'s listing on Apple Podcasts, or when/how your podcast appears in Apple Search results.','empowercast'); ?>
</p>
<div class="pp-settings-section">
    <h2><?php echo __('Program Subtitle', 'empowercast'); ?> <br /></h2>
...
<div class="pp-settings-section">
    <h2><?php echo __('Program Summary', 'empowercast'); ?></h2>
    <p class="pp-main"><?php echo __('Your summary cannot exceed 4,000 characters in length and should not include HTML, except for hyperlinks', 'empowercast'); ?></p>
    ...
    <div class="pp-settings-subsection" style="border: none;">
        <p class="pp-sub"><?php echo __('Wrap summary values with &lt;![CDATA[ ... ]]&gt; tags', 'empowercast'); ?></p>
    </div>
</div>

<div class="pp-settings-section">
    <h2><?php echo __('Episode Summary', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-main"><?php echo __('Optimize iTunes Summary from Blog Posts', 'empowercast'); ?></p>
        <p class="pp-sub"><?php echo __('Creates a friendlier view of your post/episode content.', 'empowercast'); ?></p>
    </div>
</div>
...
?>
<div class="pp-settings-section">
    <h2><?php echo __('Program Keywords', 'empowercast'); ?></h2>
    ...
    <p><?php echo __('Feature Deprecated by Apple. Keywords above are for your reference only.', 'empowercast'); ?></p>
</div>
<?php
		} // End iTunes keywords
...
$Categories = empowercast_itunes_categories(true);
$AppleCategories = empowercast_apple_categories(true);

?>
    <div class="pp-settings-section" style="display: inline-block;width: 28%;">
        <h2><?php echo __('Category', 'empowercast'); ?><span class="empowercast-required"><?php echo __('Required', 'empowercast'); ?></span></h2>
        ...
        <select <?php echo $errorClass ?>  name="Feed[apple_cat_1]" class="pp-settings-select" style="width: 80%;">
            ...
            echo '<option value="">'. __('Select Category', 'empowercast') .'</option>';
            ...
            <?php
                if($errorClass == "class='pp-form-error'") {
                    ...
                    echo __('Please enter an Apple Podcasts category to prepare yourself for the new 2019 categories', 'empowercast');
                    echo '</p>';                }
                else {
                    echo '';
                }
...
<!-- start advanced features -->
    ...
    <div class="pp-settings-section" style="display: inline-block;width: 28%;">
        ...
        ?>
        <h2><?php echo __('Category 2', 'empowercast'); ?> </h2>
    ...
    <div class="pp-settings-section" style="display: inline-block;width: 28%;">
        ...
        ?>
        <h2><?php echo __('Category 3', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-section">
        <h2><?php echo __('Author Name', 'empowercast'); ?></h2>
        ...
        <div class="pp-settings-subsection" style="border: none;">
            <p class="pp-sub"><?php echo __('Use blog post author\'s name for individual episodes.', 'empowercast'); ?></p>
        </div>
    </div>


<div class="pp-settings-section" style="display: inline-block;width: 45%;">
    <h2><?php echo __('Explicit', 'empowercast'); ?><span class="empowercast-required"><?php echo __('Required', 'empowercast'); ?></span></h2>
    <select name="Feed[itunes_explicit]" class="pp-settings-select" style="width: 80%;">
    <?php
    $explicit = array(0=> __('No option selected', 'empowercast'), 1=>__('Yes - explicit content', 'empowercast'), 2=>__('Clean - no explicit content', 'empowercast'));
	...
	<p class="description pp-settings-text"><?php echo __('Note: As of February, 2016, you must select either Yes or Clean.', 'empowercast'); ?><br /><br /></p>
</div>
<div class="pp-settings-section" style="display: inline-block;width: 45%;">
    <h2><?php echo __('Feed Type', 'empowercast'); ?></h2>
    <select name="Feed[itunes_type]" class="pp-settings-select" style="width: 80%;">
        <?php
        $types = array(''=> __('No option selected', 'empowercast'), 'episodic'=>__('Episodic (default)', 'empowercast'), 'serial'=>__('Serial', 'empowercast'));
    ...
    <p class="description pp-settings-text">
        <?php echo __('Episodic: displays latest episode first.', 'empowercast'); ?> <br />
        <?php echo __('Serial: displays latest episode last.', 'empowercast'); ?>
    </p>
</div>


    <div class="pp-settings-section">
	    <h2><?php echo __('New Feed URL', 'empowercast'); ?></h2>
		<div id="new_feed_url_step_1" style="display: <?php echo ( !empty($FeedSettings['itunes_new_feed_url']) || !empty($FeedSettings['itunes_new_feed_url_podcast'])  ?'none':'block'); ?>;">
			 <p class="pp-settings-text" style="margin-top: 5px;"><strong><a href="#" onclick="return empowercast_new_feed_url_prompt();"><?php echo __('Set iTunes New Feed URL', 'empowercast'); ?></a></strong></p>
			 <p class="pp-settings-text"><strong>
			 <?php echo __('The Apple New Feed URL option works primarily for Apple\'s Podcast directory only, and should only be used if you are unable to implement a HTTP 301 redirect.', 'empowercast'); ?>
			 <?php echo __('A 301 redirect will route <u>all podcast clients including iTunes</u> to your new feed address.', 'empowercast'); ?>
			 </strong> 
			 </p>
			 <p class="pp-settings-text">
			 <?php echo __('Learn more:', 'empowercast'); ?> <a href="https://podopshost.com/manual/syndicating-your-podcast-rss-feeds/changing-your-podcast-rss-feed-address-url/" target="_blank"><?php echo __('Changing Your Podcast RSS Feed Address (URL)', 'empowercast'); ?></a>
			</p>
		</div>
		<div id="new_feed_url_step_2" style="display: <?php echo ( !empty($FeedSettings['itunes_new_feed_url']) || !empty($FeedSettings['itunes_new_feed_url_podcast'])  ?'block':'none'); ?>;">
			<p class="pp-settings-text" style="margin-top: 5px;"><strong><?php echo __('WARNING: Changes made here are permanent. If the New Feed URL entered is incorrect, you will lose subscribers and will no longer be able to update your listing in the iTunes Store.', 'empowercast'); ?></strong></p>
			<p class="pp-settings-text"><strong><?php echo __('DO NOT MODIFY THIS SETTING UNLESS YOU ABSOLUTELY KNOW WHAT YOU ARE DOING.', 'empowercast'); ?></strong></p>
			<p class="pp-settings-text">
				<?php echo htmlspecialchars( sprintf(__('Apple recommends you maintain the %s tag in your feed for at least two weeks to ensure that most subscribers will receive the new New Feed URL.', 'empowercast'), '<itunes:new-feed-url>' ) ); ?>
			</p>
			<p class="pp-settings-text">
			<?php 
			$FeedName = __('Main RSS2 feed', 'empowercast');
			$FeedURL = get_feed_link('rss2');
			if( $cat_ID )
			{
				$category = get_category_to_edit($cat_ID);
				$FeedName = sprintf( __('%s category feed', 'empowercast'), htmlspecialchars($category->name) );
				if( !empty($General['cat_casting_podcast_feeds']) )
			...
			else if( $feed_slug )
			{
				if( !empty($General['custom_feeds'][ $feed_slug ]) )
					...
					$FeedName = __('Podcast', 'empowercast');
				$FeedName = trim($FeedName).' '.__('feed', 'empowercast');
			...
			else if( $FeedAttribs['type'] == 'ttid' )
			{
				...
				$FeedName = sprintf( __('%s taxonomy term feed', 'empowercast'), htmlspecialchars($term_object->name) );
				$FeedURL = get_term_feed_link($FeedAttribs['term_id'],$FeedAttribs['taxonomy_type'], 'rss2');
			}
			
			echo sprintf(__('The New Feed URL value below will be applied to the %s (%s).', 'empowercast'), $FeedName, $FeedURL);
?>
			</p>
			<p style="margin-bottom: 0;">
				<label class="pp-settings-label"><?php echo __('New Feed URL', 'empowercast'); ?></label>
			...
			<label class="pp-settings-label-under"><?php echo __('Leave blank for no New Feed URL', 'empowercast'); ?></label>
			
			<p class="pp-settings-text" style="margin-top: 2em;"><a href="https://podopshost.com/manual/syndicating-your-podcast-rss-feeds/changing-your-podcast-rss-feed-address-url/" target="_blank"><?php echo __('More information regarding the iTunes New Feed URL is available here.', 'empowercast'); ?></a></p>
			<p class="pp-settings-text">
<?php
			if( !$cat_ID && !$feed_slug )
			{
				if( empty($General['channels']) )
					echo sprintf(__('Please activate the \'Custom Podcast Channels\' Advanced Option to set the new-feed-url for your podcast only feed (%s)', 'empowercast'), get_feed_link('podcast') );
				else
					echo sprintf(__('Please navigate to the \'Custom Podcast Channels\' section to set the new-feed-url for your podcast only feed (%s)', 'empowercast'), get_feed_link('podcast') );
			}
?>
			</p>
		</div>
...
<h1 class="pp-heading" style="margin-bottom: 1em;"><?php echo __('Advanced Options', 'empowercast');  ?></h1>
<div id="permanent_itunes_settings" class="pp-settings-section">

	<div>
		<p class="pp-settings-text" style="margin-bottom: 1em;">
			<strong style="color: #CC0000; font-weight: bold;"><?php echo __('SETTINGS BELOW HAVE PERMANENT CONSEQUENCES.', 'empowercast'); ?></strong>
		</p>
		<p style="margin-bottom: 0;" class="pp-settings-text">
			<?php echo __('Feeds affected', 'empowercast'); ?>: 
		</p>
    ...
    <h2><?php echo __('Block', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-sub"><?php echo __('Prevent the entire podcast from appearing in the Apple Podcast directory.', 'empowercast'); ?></p>
	</div>

    <h2><?php echo __('Complete', 'empowercast'); ?></h2>
    ...
    <div class="pp-settings-subsection">
        <p class="pp-sub"><?php echo __('Indicate the completion of a podcast. Apple will no longer update your listing in the Apple Podcast directory.', 'empowercast'); ?></p>
	</div>
