...
function empowercastsubscribe_get_subscribe_page($Settings)
{
...
function empowercastsubscribe_get_itunes_url($Settings)
{
...
function empowercastsubscribe_add_guid_to_itunes_url($url, $guid)
{
...
function empowercastsubscribe_get_settings($ExtraData, $detect_category=true)
{
	...
	if( !empty($category_id) && $ExtraData['subscribe_type'] == 'category' )
	{
		...
		if( !empty($Settings) )
		{
			...
			$Settings['subscribe_page_url'] = empowercastsubscribe_get_subscribe_page($Settings);
			$Settings['itunes_url'] = empowercastsubscribe_get_itunes_url($Settings);
            ...
            $Settings['subscribe_feature_podops'] = (isset($GeneralSettings['subscribe_feature_podops']) ? $GeneralSettings['subscribe_feature_podops'] : false );
            ...
            $Settings['subscribe_feature_podops_shortcode'] = (isset($GeneralSettings['subscribe_feature_podops_shortcode']) ? $GeneralSettings['subscribe_feature_podops_shortcode'] : true );
            ...
            $Settings['subscribe_feature_podops_sidebar'] = (isset($GeneralSettings['subscribe_feature_podops_sidebar']) ? $GeneralSettings['subscribe_feature_podops_sidebar'] : false );
	...
	if( $ExtraData['subscribe_type'] == 'ttid' )
	{
		if( !empty($GeneralSettings['taxonomy_podcasting']) && !empty($taxonomy_term_id) )
		{
			$term_ID = '';
			$taxonomy_type = '';
			$Settings = get_option('empowercast_taxonomy_'. intval($taxonomy_term_id), array() );
			if( !empty($Settings) ) {
				global $wpdb;
	...
	if( $ExtraData['subscribe_type'] == 'post_type' )
	{
		if( !empty($GeneralSettings['posttype_podcasting']) )
		{
			...
			switch( $post_type )
			{
				...
				default: {
					$SettingsArray = get_option('empowercast_posttype_'.$post_type, array());
					$Settings = false;
	...
	if( !empty($Settings) )
	{
        ...
        if( !empty($FeedSettings['premium']) ) {
            ...
            $Settings['subscribe_feature_podops'] = false;
            ...
            $Settings['subscribe_feature_podops_shortcode'] = false;
            ...
            $Settings['subscribe_feature_podops_sidebar'] = false;
...
function empowercastplayer_link_subscribe_pre($content, $media_url, $ExtraData = array() )
{
	...
	$SubscribeSettings = empowercastsubscribe_get_settings( $ExtraData, $detect_category );
	...
	if( !empty($itunes_url) && !empty($SubscribeSettings['subscribe_feature_apple']) ) {
		$guid = get_the_guid();
		$links_array[] = "<a href=\"".  htmlspecialchars( empowercastsubscribe_add_guid_to_itunes_url($itunes_url, $guid) ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_itunes\" target=\"_blank\" title=\"". __('Subscribe on Apple Podcasts', 'empowercast') ."\" rel=\"nofollow\">". __('Apple Podcasts','empowercast') ."</a>".PHP_EOL_WEB;
	}
    ...
    if( !empty($SubscribeSettings['subscribe_feature_spotify']) && !empty($SubscribeSettings['spotify_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['spotify_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_spotify\" target=\"_blank\" title=\"". __('Subscribe on Spotify', 'empowercast') ."\" rel=\"nofollow\">". __('Spotify','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_amazon']) && !empty($SubscribeSettings['amazon_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['amazon_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_amazon\" target=\"_blank\" title=\"". __('Subscribe on Amazon Music', 'empowercast') ."\" rel=\"nofollow\">". __('Amazon Music','empowercast') ."</a>".PHP_EOL_WEB;
    }
	...
	if( preg_match('/^(https?:\/\/)(.*)$/i', $feed_url, $matches ) && !empty($SubscribeSettings['subscribe_feature_android']) ) {
		    ...
		    $links_array[] = "<a href=\"".  htmlspecialchars($android_url) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_android\" target=\"_blank\" title=\"". __('Subscribe on Android', 'empowercast') ."\" rel=\"nofollow\">". __('Android','empowercast') ."</a>".PHP_EOL_WEB;
	}
    ...
    if( !empty($SubscribeSettings['subscribe_feature_pandora']) && !empty($SubscribeSettings['pandora_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['pandora_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_pandora\" target=\"_blank\" title=\"". __('Subscribe on Pandora', 'empowercast') ."\" rel=\"nofollow\">". __('Pandora','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_iheart']) && !empty($SubscribeSettings['iheart_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['iheart_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_iheart\" target=\"_blank\" title=\"". __('Subscribe on iHeartRadio', 'empowercast') ."\" rel=\"nofollow\">". __('iHeartRadio','empowercast') ."</a>".PHP_EOL_WEB;
    }

    if( !empty($SubscribeSettings['subscribe_feature_podops']) && !empty($SubscribeSettings['podops_url']) )
    {
        $SubscribeSettings['podops_url'] = trim($SubscribeSettings['podops_url']);
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['podops_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_podops\" target=\"_blank\" title=\"". __('Subscribe on Podops', 'empowercast') ."\" rel=\"nofollow\">". __('Podops','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_jiosaavn']) && !empty($SubscribeSettings['jiosaavn_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['jiosaavn_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_jiosaavn\" target=\"_blank\" title=\"". __('Subscribe on JioSaavn', 'empowercast') ."\" rel=\"nofollow\">". __('JioSaavn','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_podchaser']) && !empty($SubscribeSettings['podchaser_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['podchaser_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_podchaser\" target=\"_blank\" title=\"". __('Subscribe on Podchaser', 'empowercast') ."\" rel=\"nofollow\">". __('Podchaser','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_gaana']) && !empty($SubscribeSettings['gaana_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['gaana_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_gaana\" target=\"_blank\" title=\"". __('Subscribe on Gaana', 'empowercast') ."\" rel=\"nofollow\">". __('Gaana','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_pcindex']) && !empty($SubscribeSettings['pcindex_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['pcindex_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_pcindex\" target=\"_blank\" title=\"". __('Subscribe on Podcast Index', 'empowercast') ."\" rel=\"nofollow\">". __('Podcast Index','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( preg_match('/^(https?:\/\/)(.*)$/i', $feed_url, $matches ) && !empty($SubscribeSettings['subscribe_feature_email']) ) {
        ...
        $links_array[] = "<a href=\"" . htmlspecialchars($email_url) . "\" class=\"empowercast_link_subscribe empowercast_link_subscribe_email\" target=\"_blank\" title=\"" . __('Subscribe by Email', 'empowercast') . "\" rel=\"nofollow\">" . __('Email', 'empowercast') . "</a>" . PHP_EOL_WEB;
    }
	...
	if( !empty($SubscribeSettings['subscribe_feature_tunein']) && !empty($SubscribeSettings['tunein_url']) )
	{
		...
		$links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['tunein_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_tunein\" target=\"_blank\" title=\"". __('Subscribe on TuneIn', 'empowercast') ."\" rel=\"nofollow\">". __('TuneIn','empowercast') ."</a>".PHP_EOL_WEB;
	}
    ...
    if( !empty($SubscribeSettings['subscribe_feature_deezer']) && !empty($SubscribeSettings['deezer_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['deezer_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_deezer\" target=\"_blank\" title=\"". __('Subscribe on Deezer', 'empowercast') ."\" rel=\"nofollow\">". __('Deezer','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_anghami']) && !empty($SubscribeSettings['anghami_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['anghami_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_anghami\" target=\"_blank\" title=\"". __('Subscribe on Anghami', 'empowercast') ."\" rel=\"nofollow\">". __('Anghami','empowercast') ."</a>".PHP_EOL_WEB;
    }
    ...
    if( !empty($SubscribeSettings['subscribe_feature_youtube']) && !empty($SubscribeSettings['youtube_url']) )
    {
        ...
        $links_array[] = "<a href=\"".  esc_attr($SubscribeSettings['youtube_url'] ) ."\" class=\"empowercast_link_subscribe empowercast_link_subscribe_youtube\" target=\"_blank\" title=\"". __('Subscribe on Youtube Music', 'empowercast') ."\" rel=\"nofollow\">". __('Youtube Music','empowercast') ."</a>".PHP_EOL_WEB;
    }

    if (!empty($SubscribeSettings['subscribe_feature_rss'])) {
        $links_array[] = "<a href=\"" . htmlspecialchars($feed_url) . "\" class=\"empowercast_link_subscribe empowercast_link_subscribe_rss\" target=\"_blank\" title=\"" . __('Subscribe via RSS', 'empowercast') . "\" rel=\"nofollow\">" . __('RSS', 'empowercast') . "</a>" . PHP_EOL_WEB;
    }

    if( !empty($SubscribeSettings['subscribe_page_url']) )
	{
		$label = (empty($SubscribeSettings['subscribe_page_link_text'])?__('More', 'empowercast'):$SubscribeSettings['subscribe_page_link_text']);
		$links_array[] = "<a href=\"{$SubscribeSettings['subscribe_page_url']}\" class=\"empowercast_link_subscribe empowercast_link_subscribe_more\" target=\"_blank\" title=\"". htmlspecialchars($label) ."\" rel=\"nofollow\">". htmlspecialchars($label) ."</a>".PHP_EOL_WEB;
	}
	
	$content .= implode(' '.EMPOWERCAST_LINK_SEPARATOR .' ', $links_array);
	return $content;
}

function empowercastplayer_link_subscribe_post($content, $media_url, $ExtraData = array() )
{
	if( !empty($content) )
	{
		$GeneralSettings = get_option('empowercast_general', array());
		
		$label = __('Subscribe:', 'empowercast');
		...
		$return = '<p class="empowercast_links empowercast_subscribe_links">'. $prefix . $content . '</p>';
		return $return;
	}
	return $content;
}

function empowercast_subscribe_shortcode( $attr ) {
	...
	extract( shortcode_atts( array(
		'channel'=>'', // Used for EmpowerCast Podcast Channels
		'slug' => '', // Used for EmpowerCast (alt for 'channel')
		'feed' => '', // Used for EmpowerCast (alt for 'channel')
		'post_type' => 'post', // Used for EmpowerCast 
		'category'=>'', // Used for EmpowerCast (specify category ID, name or slug)
		'term_taxonomy_id'=>'', // Used for EmpowerCast (specify term taxonomy ID)
		//'term_id'=>'', // Used for EmpowerCast (specify term ID, name or slug)
		//'taxonomy'=>'', // Used for EmpowerCast (specify taxonomy name)
	...
	*/
	...
	{
		$Settings = empowercastsubscribe_get_settings(  array('feed'=>$attr['slug'], 'taxonomy_term_id'=>$attr['term_taxonomy_id'], 'cat_id'=>$category_id, 'post_type'=>$attr['post_type'], 'subscribe_type'=>$subscribe_type), false );
	}
    ...
    if (defined('WP_DEBUG')) {
        if (WP_DEBUG) {
            wp_enqueue_style('empowercast-subscribe-style-modern', plugin_dir_url(__FILE__) . 'css/subscribe.css', array(), EMPOWERCAST_VERSION);
        } else {
            wp_enqueue_style('empowercast-subscribe-style-modern', plugin_dir_url(__FILE__) . 'css/subscribe.min.css', array(), EMPOWERCAST_VERSION);
        }
    } else {
        wp_enqueue_style('empowercast-subscribe-style-modern', plugin_dir_url(__FILE__) . 'css/subscribe.min.css', array(), EMPOWERCAST_VERSION);
    }
	...
	{
		$apple_id = empowercast_get_apple_id($Settings['itunes_url'], true);
	...
	$EmpowerCastSettings = get_option('empowercast_general', array());
	$Settings['subscribe_widget_shape'] = ( empty($EmpowerCastSettings['subscribe_widget_shape']) || $EmpowerCastSettings['subscribe_widget_shape'] == 'squared' ? '-sq': '');
	$Settings['subscribe_no_important_styling'] = (!empty($EmpowerCastSettings['subscribe_no_important_styling']) ? $EmpowerCastSettings['subscribe_no_important_styling'] : '' );
	return empowercast_do_subscribe_widget($Settings, $EmpowerCastSettings);
}

add_shortcode( 'empowercastsubscribe', 'empowercast_subscribe_shortcode' );
add_shortcode( 'empowercast_subscribe', 'empowercast_subscribe_shortcode' );
	
require_once( EMPOWERCAST_ABSPATH . '/class.empowercast-subscribe-widget.php' );

function empowercast_do_subscribe_widget($settings, $EmpowerCastSettings)
{
    //ob_start();
	...
	{
		$settings['image_url'] = empowercast_get_root_url() . 'itunes_default.jpg'; // Default EmpowerCast image used in this case.
	}
    ...
    if( !empty($settings['title']) ) {
        ...
        $html .= '<div class="pp-sub-t">' . __('Subscribe to podcast', 'empowercast') . '</div>';
    }
	...
	$link_counter = 0;
			if( !empty($settings['itunes_url']) &&  !empty($settings['subscribe_feature_apple_shortcode']) ) {
			    $link_counter++;
				$html .= '<a href="'.  esc_url( $settings['itunes_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-itunes" target="_blank" title="'.  esc_attr( __('Subscribe on Apple Podcasts', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Apple Podcasts', 'empowercast') ) .'</a>';
			}

            if(  !empty($settings['subscribe_feature_spotify_shortcode']) && !empty($settings['spotify_url']) )
            {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['spotify_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-spotify" target="_blank" title="'.  esc_attr( __('Subscribe on Spotify', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Spotify', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_amazon_shortcode']) &&  !empty($settings['amazon_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['amazon_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-amazon" target="_blank" title="'.  esc_attr( __('Subscribe on Amazon Music', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Amazon Music', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_android_shortcode']) &&  preg_match('/^(https?:\/\/)(.*)$/i', $settings['feed_url'], $matches ) ) {
                $android_url =  $matches[1] . 'subscribeonandroid.com/' . $matches[2];
                $link_counter++;
                $html .= '<a href="'.  esc_url( $android_url ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-android" target="_blank" title="'.  esc_attr( __('Subscribe on Android', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Android', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_pandora_shortcode']) &&  !empty($settings['pandora_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['pandora_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-pandora" target="_blank" title="'.  esc_attr( __('Subscribe on Pandora', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Pandora', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_iheart_shortcode']) &&  !empty($settings['iheart_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['iheart_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-iheartradio" target="_blank" title="'.  esc_attr( __('Subscribe on iHeartRadio', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('iHeartRadio', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_podops_shortcode']) &&  !empty($settings['podops_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['podops_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-podops" target="_blank" title="'.  esc_attr( __('Subscribe on Podops', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Podops', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_jiosaavn_shortcode']) &&  !empty($settings['jiosaavn_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['jiosaavn_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-jiosaavn" target="_blank" title="'.  esc_attr( __('Subscribe on JioSaavn', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('JioSaavn', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_podchaser_shortcode']) &&  !empty($settings['podchaser_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['podchaser_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-podchaser" target="_blank" title="'.  esc_attr( __('Subscribe on Podchaser', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Podchaser', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_gaana_shortcode']) &&  !empty($settings['gaana_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['gaana_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-gaana" target="_blank" title="'.  esc_attr( __('Subscribe on Gaana', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Gaana', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_pcindex_shortcode']) &&  !empty($settings['pcindex_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['pcindex_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-pcindex" target="_blank" title="'.  esc_attr( __('Subscribe on Podcast Index', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Podcast Index', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_email_shortcode']) &&  preg_match('/^(https?:\/\/)(.*)$/i', $settings['feed_url'], $matches ) ) {
                $link_counter++;
                $email_url =  $matches[1] . 'subscribebyemail.com/' . $matches[2];
                $html .= '<a href="'.  esc_url( $email_url ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-email" target="_blank" title="'.  esc_attr( __('Subscribe by Email', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('by Email', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_tunein_shortcode']) &&  !empty($settings['tunein_url']) )
            {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['tunein_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-tunein" target="_blank" title="'.  esc_attr( __('Subscribe on TuneIn', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('TuneIn', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_deezer_shortcode']) &&  !empty($settings['deezer_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['deezer_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-deezer" target="_blank" title="'.  esc_attr( __('Subscribe on Deezer', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Deezer', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_anghami_shortcode']) &&  !empty($settings['anghami_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['anghami_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-anghami" target="_blank" title="'.  esc_attr( __('Subscribe on Anghami', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Anghami', 'empowercast') ) .'</a>';
            }

            if( !empty($settings['subscribe_feature_youtube_shortcode']) &&  !empty($settings['youtube_url']) ) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['youtube_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-youtube" target="_blank" title="'.  esc_attr( __('Subscribe on Youtube Music', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('Youtube Music', 'empowercast') ) .'</a>';
            }
            ...
            if (!empty($settings['subscribe_feature_rss_shortcode'])) {
                $link_counter++;
                $html .= '<a href="'.  esc_url( $settings['feed_url'] ) .'" class="pp-sub-btn'. htmlspecialchars($settings['subscribe_widget_shape']) .' pp-sub-rss" target="_blank" title="'.  esc_attr( __('Subscribe via RSS', 'empowercast') ) .'"><span class="pp-sub-ic"></span>'.  esc_html( __('RSS', 'empowercast') ) .'</a>';
            }
...
function empowercast_do_subscribe_sidebar_widget($settings)
{
	if( empty($settings['feed_url']) )
	{
		return '';
...
*/
	...
	$html .= '<div class="pp-ssb-widget pp-ssb-widget-modern' . $important_tags_class .'">';
		if( !empty($settings['itunes_url']) && !empty($settings['subscribe_feature_apple_sidebar']) ) {
			$html .= '<a href="'.  esc_url( $settings['itunes_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-itunes" target="_blank" title="'.  esc_attr( __('Subscribe on Apple Podcasts', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Apple Podcasts', 'empowercast') ) .'</span></a>';
		}
        ...
        if(  !empty($settings['subscribe_feature_spotify_sidebar']) && !empty($settings['spotify_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['spotify_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-spotify" target="_blank" title="'.  esc_attr( __('Subscribe on Spotify', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Spotify', 'empowercast') ) .'</span></a>';
        }

        if(  !empty($settings['subscribe_feature_amazon_sidebar']) && !empty($settings['amazon_url']) ) {
            $settings['amazon_url'] = trim($settings['amazon_url']);
            $html .= '<a href="'.  esc_url( $settings['amazon_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-amazon" target="_blank" title="'.  esc_attr( __('Subscribe on Amazon Music', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Amazon Music', 'empowercast') ) .'</span></a>';
        }

        if( preg_match('/^(https?:\/\/)(.*)$/i', $settings['feed_url'], $matches ) && !empty($settings['subscribe_feature_android_sidebar']) ) {
            $android_url =  $matches[1] . 'subscribeonandroid.com/' . $matches[2];
            $html .= '<a href="'.  esc_url( $android_url ) .'" class="pp-ssb-btn'.$settings['modern_style'].'  '.$settings['modern_direction'].' pp-ssb-android" target="_blank" title="'.  esc_attr( __('Subscribe on Android', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Android', 'empowercast') ) .'</span></a>';
        }
        ...
        if(  !empty($settings['subscribe_feature_pandora_sidebar']) && !empty($settings['pandora_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['pandora_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-pandora" target="_blank" title="'.  esc_attr( __('Subscribe on Pandora', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Pandora', 'empowercast') ) .'</span></a>';
        }

        if(  !empty($settings['subscribe_feature_iheart_sidebar']) && !empty($settings['iheart_url']) ) {
            $settings['iheart_url'] = trim($settings['iheart_url']);
            $html .= '<a href="'.  esc_url( $settings['iheart_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-iheartradio" target="_blank" title="'.  esc_attr( __('Subscribe on iHeartRadio', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('iHeartRadio', 'empowercast') ) .'</span></a>';
        }

        if( !empty($settings['subscribe_feature_podops_sidebar']) && !empty($settings['podops_url']) ) {
            $settings['podops_url'] = trim($settings['podops_url']);
            $html .= '<a href="'.  esc_url( $settings['podops_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-podops" target="_blank" title="'.  esc_attr( __('Subscribe on Podops', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Podops', 'empowercast') ) .'</span></a>';
        }
        ...
        if(  !empty($settings['subscribe_feature_jiosaavn_sidebar']) && !empty($settings['jiosaavn_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['jiosaavn_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-jiosaavn" target="_blank" title="'.  esc_attr( __('Subscribe on JioSaavn', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('JioSaavn', 'empowercast') ) .'</span></a>';
        }
        ...
        if(  !empty($settings['subscribe_feature_podchaser_sidebar']) && !empty($settings['podchaser_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['podchaser_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-podchaser" target="_blank" title="'.  esc_attr( __('Subscribe on Podchaser', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Podchaser', 'empowercast') ) .'</span></a>';
        }

        if(  !empty($settings['subscribe_feature_gaana_sidebar']) && !empty($settings['gaana_url']) ) {
            $settings['gaana_url'] = trim($settings['gaana_url']);
            $html .= '<a href="'.  esc_url( $settings['gaana_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-gaana" target="_blank" title="'.  esc_attr( __('Subscribe on Gaana', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Gaana', 'empowercast') ) .'</span></a>';
        }
        ...
        if(  !empty($settings['subscribe_feature_pcindex_sidebar']) && !empty($settings['pcindex_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['pcindex_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-pcindex" target="_blank" title="'.  esc_attr( __('Subscribe on Podcast Index', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Podcast Index', 'empowercast') ) .'</span></a>';
        }

        if( preg_match('/^(https?:\/\/)(.*)$/i', $settings['feed_url'], $matches )  ) {
            if( !empty($settings['subscribe_feature_email_sidebar']) ) {
                $email_url =  $matches[1] . 'subscribebyemail.com/' . $matches[2];
                $html .= '<a href="'.  esc_url( $email_url ) .'" class="pp-ssb-btn'.$settings['modern_style'].'  '.$settings['modern_direction'].'  pp-ssb-email" target="_blank" title="'.  esc_attr( __('Subscribe by Email', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('by Email', 'empowercast') ) .'</span></a>';
            }
        }

        if( !empty($settings['subscribe_feature_tunein_sidebar']) && !empty($settings['tunein_url']) ) {
            $settings['tunein_url'] = trim($settings['tunein_url']);
            $html .= '<a href="'.  esc_url( $settings['tunein_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-tunein" target="_blank" title="'.  esc_attr( __('Subscribe on TuneIn', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('TuneIn', 'empowercast') ) .'</span></a>';
        }

        if(  !empty($settings['subscribe_feature_deezer_sidebar']) && !empty($settings['deezer_url']) ) {
            $settings['deezer_url'] = trim($settings['deezer_url']);
            $html .= '<a href="'.  esc_url( $settings['deezer_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-deezer" target="_blank" title="'.  esc_attr( __('Subscribe on Deezer', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Deezer', 'empowercast') ) .'</span></a>';
        }
        ...
        if(  !empty($settings['subscribe_feature_anghami_sidebar']) && !empty($settings['anghami_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['anghami_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-anghami" target="_blank" title="'.  esc_attr( __('Subscribe on Anghami', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Anghami', 'empowercast') ) .'</span></a>';
        }
        ...
        if(  !empty($settings['subscribe_feature_youtube_sidebar']) && !empty($settings['youtube_url']) ) {
            ...
            $html .= '<a href="'.  esc_url( $settings['youtube_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-youtube" target="_blank" title="'.  esc_attr( __('Subscribe on Youtube Music', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('Youtube Music', 'empowercast') ) .'</span></a>';
        }

        if (!empty($settings['subscribe_feature_rss_sidebar'])) {
            $html .= '<a href="' . esc_url($settings['feed_url']) . '" class="pp-ssb-btn' . $settings['modern_style'] . ' ' . $settings['modern_direction'] . ' pp-ssb-rss" target="_blank" title="' . esc_attr(__('Subscribe via RSS', 'empowercast')) . '"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">' . esc_html(__('RSS', 'empowercast')) . '</span></a>';
        }
		
		if( !empty($settings['subscribe_page_url']) )
			$html .= '<a href="'.  esc_url( $settings['subscribe_page_url'] ) .'" class="pp-ssb-btn'.$settings['modern_style'].' '.$settings['modern_direction'].' pp-ssb-more" target="_blank" title="'.  esc_attr( __('More Subscribe Options', 'empowercast') ) .'"><span class="pp-ssb-ic"></span><span class="pp-ssb-text">'.  esc_html( __('More Subscribe Options', 'empowercast') ) .'</span></a>';
	$html .= '</div>';
