<?php
/**
 * Class EmpowerCastPostToSocial
 */

define('EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET', 0);
define('EMPOWERCAST_POSTED_STATUS_SUCCESS', 1);
define('EMPOWERCAST_POSTED_STATUS_FAILED', 2);
define('EMPOWERCAST_POSTED_STATUS_INVALID_CREDENTIALS', 3);
define('EMPOWERCAST_POSTED_STATUS_NO_CREDENTIALS', 4);
define('EMPOWERCAST_POSTED_STATUS_NOT_AUDIO', 5);
define('EMPOWERCAST_POSTED_STATUS_CONVERSION_FAILED', 6);
define('EMPOWERCAST_POSTED_STATUS_PROGRAM_ID_NOT_MATCHED', 7);
define('EMPOWERCAST_POSTED_STATUS_NOT_SCHEDULED', 8);
...
class EmpowerCastPostToSocial {
	...
	// constructor
	/**
	 * EmpowerCastPostToSocial constructor.
	 */
	...
	function my_add_metabox() {
		add_meta_box( 'pps_pts', __( 'EmpowerCast Post to Social', 'empowercast' ),  array($this, 'display_my_metabox'), 'post', 'side', 'default' );
	}
	...
	function display_my_metabox() {
		...
		if ( get_post_status( get_the_ID() ) == 'publish' ) {
			...
			$EpisodeData = empowercast_get_enclosure_data($post_id);
			if( !empty($EpisodeData) && parse_url($EpisodeData['url'], PHP_URL_HOST) == 'media.podopshost.com' ) {
				add_thickbox();
				echo "<strong styleX='font-size: 115%; display: block; text-align: center;'><a class='thickbox button button-primary button-large' href='admin.php?action=empowercast-jquery-pts&width=600&height=550&post_id={$post_id}&guid={$guid}&TB_iframe=true' target='blank' title='Post to Social'>Post to Social</a></strong>";
				$can_post = true;
				echo "<br><br>\n";
			}
		}
		...
		if ( get_post_status( get_the_ID() ) == 'publish' ) {
			if ( get_post_meta( get_the_ID(), 'pts_scheduled', true ) ) {
				_e( 'Episode posted to social sites.', 'empowercast' );
			}
			else if( $can_post == false ) {
			
				_e( 'No podcast episode available in this post to send to social sites.', 'empowercast' );
			} else if (parse_url($EpisodeData['url'], PHP_URL_HOST) == 'media.podopshost.com') {
				_e( 'Nothing posted yet.', 'empowercast' );
				echo ' ';
				echo "<a class='thickbox' href='admin.php?action=empowercast-jquery-pts&width=600&height=550&post_id={$post_id}&guid={$guid}&TB_iframe=true' target='blank'>Post Now!</a>";
			}
			else {
			    _e('The media file must be hosted on Podops to post to social sites.', 'empowercast');
            }
		}
		else {
		...
		echo 'About: Post podcast episodes to Twitter, YouTube and Facebook using Podops\'s <a href="https://create.podopshost.com/resources/podcast-media-hosting/post-to-social/" target="_blank">Post to Social</a> service.';
		echo "</p>";
	}
	...
	function do_pings() {
		...
		$enclosure_data = empowercast_get_enclosure_data( $post_id );
...
function callUpdateListing( $post_id, $guid ) {
	...
	if ( empty( $subtitle ) ) {
		$subtitle = empowercast_get_the_exerpt( false, true, $post_id );
	}
	if ( empty( $subtitle ) ) {
    ...
    if ($creds) {
        ...
        foreach ($api_url_array as $api_url) {
            $response = empowercast_remote_fopen("{$api_url}social/{$program_keyword}/update-listing.json", $Settings['podops_auth'], json_encode($post_params));
...
function callGetSocialOptions( $program_keyword, $podcast_id ) {
    ...
    if ($creds) {
        ...
        foreach ($api_url_array as $api_url) {
            $response = empowercast_remote_fopen("{$api_url}social/{$program_keyword}/get-social-options.json?podcast_id={$podcast_id}", $Settings['podops_auth']);
            if ($response) {
                break;
            }
        }
...
function displayStatus($isPostedArray)
{
    ...
    foreach($tempStatusArray as $isPosted){
        ...
        if ($isPosted == EMPOWERCAST_POSTED_STATUS_SUCCESS) { ?>
            <span class="label label-success">Posted!</span>
            <?php return;

        } else if ($isPosted == EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET) { ?>
        ...
        } else if ($isPosted == EMPOWERCAST_POSTED_STATUS_NOT_SCHEDULED) { ?>

        <?php } else if ($isPosted == EMPOWERCAST_POSTED_STATUS_CONVERSION_FAILED) { ?>
            <span class="label label-danger">Error occurred: Video creation failed</span>

        <?php } else if($isPosted == EMPOWERCAST_POSTED_STATUS_NOT_AUDIO) { ?>
            <span class="label label-danger">Error occurred: Mp3 file required for posting to Youtube</span>

        <?php } else if ($isPosted == EMPOWERCAST_POSTED_STATUS_INVALID_CREDENTIALS || $isPosted == EMPOWERCAST_POSTED_STATUS_NO_CREDENTIALS) { ?>
            <span class="label label-danger">Error occurred: Please re-link your account</span>

        <?php } else if ($isPosted > EMPOWERCAST_POSTED_STATUS_SUCCESS) { ?>
...
function empowercast_ajax_pts($Settings)
{
	empowercast_admin_jquery_header( __( 'Post to Social', 'empowercast' ) );

	if ( !current_user_can('publish_posts' ) ) {
		empowercast_page_message_add_notice( __( 'You do not have sufficient permission to do this.', 'empowercast' ) );
	...
	if ( empty( $Settings['podops_program_keyword'] ) ) {
		empowercast_page_message_add_notice( __( 'You must connect your Podops account and set up a program.', 'empowercast' ) );
		empowercast_page_message_print();
		?>
		<p style="text-align: center;"><a href="#" onclick="self.parent.tb_remove();"><?php echo __( 'Close', 'empowercast' ); ?></a></p>
		<?php
	...
	// make sure the podcast episode is in the Podops directory using the `update-listing` api call
	...
	if ( isset( $response['error'] ) ) {
		empowercast_page_message_add_notice( $response['error'] );
		empowercast_page_message_print();
		?>
		<p style="text-align: center;"><a href="#" onclick="self.parent.tb_remove();"><?php echo __( 'Close', 'empowercast' ); ?></a></p>
		<?php
	...
	if ( isset( $response['warnings'] ) ) {
		empowercast_page_message_add_notice( $response['warnings'] );
		empowercast_page_message_print();
	}
	...
	if ( isset( $response['error'] ) ) {
		empowercast_page_message_add_notice( __( 'There was a problem fetching your post to social settings. ', 'empowercast' ) .$response['error'] );
		empowercast_page_message_print();
		?>
		<p style="text-align: center;"><a href="#" onclick="self.parent.tb_remove();"><?php echo __( 'Close', 'empowercast' ); ?></a></p>
		<?php
	...
	<form action="admin.php?action=empowercast-jquery-pts-post" method="POST" id="pts_form" name="pts_form">
	...
	<?php
        if (empty($response['accounts'])) {
            ...
            echo '<p>Visit the <a href="https://publish.podopshost.com/social/" target="_blank">Podcaster Dashboard</a> to link your social accounts</p>';
        }
        if (!empty($response['accounts']['Twitter'])){
            ...
            foreach ($response['accounts']['Twitter'] as $meta_id => $account){
                ...
                <label  class="checkbox-inline"><input type="checkbox" name="twitter_meta[]"
                        value="<?php echo $meta_id ?>" <?php echo ($isPostedToTwitter[0] < EMPOWERCAST_POSTED_STATUS_FAILED || $isPostedToTwitter[0] == EMPOWERCAST_POSTED_STATUS_NOT_SCHEDULED) ? 'checked' : '';?> <?php echo ($isPostedToTwitter[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS) ? 'disabled' : '';?>> <?php echo "@" . $account['twitter_handle']?></label>
            ...
            <textarea onKeyUp=check_length(this.form); class="form-control" rows="2" name="twitter_content" id="twitter_content"
                      size="<?php echo 280 - (strlen($response['settings']['link'])); ?>" placeholder="What's happening?" <?php echo ($isPostedToTwitter[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS || $isPostedToTwitter[0] == EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET) ? 'readonly' : '';?>
        ...
        if (!empty($response['accounts']['Youtube'])){
            ...
            foreach ($response['accounts']['Youtube'] as $meta_id => $account){ ?>
                ...
                <label class="checkbox-inline"><input type="checkbox" name="youtube_meta[]"
                        value="<?php echo $meta_id ?>" <?php echo ($isPostedToYoutube[0] < EMPOWERCAST_POSTED_STATUS_FAILED || $isPostedToYoutube[0] == EMPOWERCAST_POSTED_STATUS_NOT_SCHEDULED) ? 'checked' : '';?> <?php echo ($isPostedToYoutube[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS) ? 'disabled' : '';?>> <?php echo "as " . $account['youtube_name']?></label>
            ...
            <textarea class="form-control" rows="1" id="youtube_title" name="youtube_title"
                      placeholder="Video title" <?php echo ($isPostedToYoutube[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS || $isPostedToYoutube[0] == EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET) ? 'readonly' : '';?>><?php echo (isset($response['accounts']['youtube_posted_array']['youtube_title']) ? $response['accounts']['youtube_posted_array']['youtube_title'] : '')?></textarea>
            ...
            <textarea class="form-control" rows="3" id="youtube_description" name="youtube_description"
                      placeholder="Youtube description" <?php echo ($isPostedToYoutube[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS || $isPostedToYoutube[0] == EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET) ? 'readonly' : '';?>><?php echo (isset($response['accounts']['youtube_posted_array']['youtube_description']) ? $response['accounts']['youtube_posted_array']['youtube_description'] : '')?></textarea>
        <?php }
        ...
        if (!empty($response['accounts']['Facebook'])){
            ...
            foreach ($response['accounts']['Facebook'] as $meta_id => $account){
                foreach ($account['pages'] as $page){ ?>
                    ...
                    <label  class="checkbox-inline"><input type="checkbox" name="facebook_meta[<?php echo $meta_id ?>][]"
                           value="<?php echo $page['name'] ?>" <?php echo ($isPostedToFacebook[0] < EMPOWERCAST_POSTED_STATUS_FAILED || $isPostedToFacebook[0] == EMPOWERCAST_POSTED_STATUS_NOT_SCHEDULED) ? 'checked' : '';?> <?php echo ($isPostedToFacebook[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS) ? 'disabled' : '';?>> <?php echo $page['name'] . " (as " . $account['social_name'] . ")"?></label>
            ...
            <textarea class="form-control" rows="1" id="facebook_link" name="facebook_link"
                      placeholder="Link your podcast here" <?php echo ($isPostedToFacebook[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS || $isPostedToFacebook[0] == EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET) ? 'readonly' : '';?>><?php echo (isset($response['accounts']['facebook_posted_array']['link_to_podcast']) ? $response['accounts']['facebook_posted_array']['link_to_podcast'] : $response['settings']['link']) ?></textarea>
            <br>
            ...
            <textarea class="form-control" rows="3" id="facebook_description" name="facebook_description"
                      placeholder="What's on your mind?" <?php echo ($isPostedToFacebook[0] == EMPOWERCAST_POSTED_STATUS_SUCCESS || $isPostedToFacebook[0] == EMPOWERCAST_POSTED_STATUS_NOT_POSTED_YET) ? 'readonly' : '';?>><?php echo (isset($response['accounts']['facebook_posted_array']['facebook_description']) ? $response['accounts']['facebook_posted_array']['facebook_description'] : '')?></textarea>

        <?php }
	?>
		<hr>
        <small>Disclaimer: By hitting "Post to Selected Media Accounts" you are agreeing to allow Podops to post the
    ...
    <a href="#" class="btn btn-sm btn-default"
       onclick="self.parent.tb_remove();"><?php echo __( 'Cancel', 'empowercast' ); ?></a>
    </form>
	<?php
	empowercast_admin_jquery_footer();
}

function empowercast_ajax_pts_post($Settings)
{
	empowercast_admin_jquery_header( __( 'Post to Social', 'empowercast' ) );
    ...
    if ($creds) {
        ...
        foreach ($api_url_array as $api_url) {
            $response = empowercast_remote_fopen("{$api_url}social/{$program_keyword}/post.json", $Settings['podops_auth'], json_encode($post_params));
            if ($response) {
                break;
            }
        }
	...
	if ( $response['status'] == 'success' ) {
		empowercast_page_message_add_notice( __( 'Posting to social been scheduled! Please allow up to an hour to post.', 'empowercast' ) );
		empowercast_page_message_print();
...
$empowercast_PTS = new EmpowerCastPostToSocial();
