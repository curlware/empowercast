...
class EmpowercastNetworkDataBus{
     ...
     * @param $creds array of oauth credentials for podops api
    ...
    static function getNetworksInAccount($creds)
    {
        ...
        $requestUrl = '/2/empowercast/network?cache=' . md5( rand(0, 999) . time()) ;
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl,  true);
    }
    ...
    static function getSpecificNetworkInAccount($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        if (isset($networkInfo['network_id'])) {
            ...
            $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id']. '?cache=' . md5( rand(0, 999) . time()) ;
            $props = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl, $needDirectAPI);
            return $props;
        }
        return false;
    }
    ...
    static function getProgramsInNetwork($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        ...
        $requestUrl = '/2/empowercast/network/' . $networkInfo['network_id'] . '/programs?cache=' . md5( rand(0, 999) . time()) ;
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl, $needDirectAPI);
    }
    ...
    {
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/applicant/changestatus?action=' . $appAction['appAction'] . '&applicantId=' . $appAction['applicantId'];
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl,  true);
    }
    ...
    static function getListsInNetwork($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        ...
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/?cache=' . md5( rand(0, 999) . time()) ;
        $props = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl, $needDirectAPI);
        return $props;
    }
    ...
    static function createNewList($apiUrl, $creds, $networkInfo, $create)
    {
        ...
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/new?title=' . $create['newListTitle'] . '&description=' . $create['newListDescription'];
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);
    }
    ...
    {
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/' . $networkInfo['list_id'] . '/update-programs?';
    ...
    {
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/' . $networkInfo['list_id'] . '/add-program-to-list?';
    ...
    {
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/' . $networkInfo['list_id'] . '/delete';
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);
    }
    ...
    static function getApplicantsInNetwork($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        ...
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/applicant?cache=' . md5( rand(0, 999) . time() );
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl, $needDirectAPI);
    }
    ...
    static function getSpecificListInNetwork ($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        if (!empty($networkInfo['network_id']) && !empty ($networkInfo['list_id'])) {
            ...
            $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/' . $networkInfo['list_id'] . '/programs?cache=' . md5( rand(0, 999) . time()) ;
            $props = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl, $needDirectAPI = true);
            if (!empty($props['list_info']['list_title']) && !empty($props['list_info']['list_description'])) {
                EmpowerCastNetwork::insertOption('list_title', $props['list_info']['list_title']);
                EmpowerCastNetwork::insertOption('list_description', $props['list_info']['list_description']);
            }
            return $props;
        }
        return false;
    ...
    static function getSpecificProgramInNetwork($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        ...
        if (!empty($networkInfo['network_id']) && !empty($networkInfo['program_id']) ) {
            $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/programs/' . $networkInfo['program_id'] . '?cache=' . md5( rand(0, 999) . time()) ;
            $props = EmpowercastNetworkDataBus::getCacheOrCallAPI(false, $cacheName, $requestUrl, $needDirectAPI);
            if (!empty($props['program_info']['program_id']) && !empty($props['program_info']['program_title'])) {
                EmpowerCastNetwork::insertOption('program_id', $props['program_info']['program_id']);
                EmpowerCastNetwork::insertOption('program_title', $props['program_info']['program_title']);
            }
            return $props;
        }
        return false;
    ...
    static function removeSpecificProgramInNetwork($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        $cacheName = 'ppn-cache n-'.$networkInfo['network_id'].'-p-'.$networkInfo['program_id'];
        if (!empty($networkInfo['network_id']) && !empty($networkInfo['program_id']) ) {
            $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/programs/' . $networkInfo['program_id'] . '/delete';
            $props = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, $cacheName, $requestUrl, $needDirectAPI);
            if (!empty($props['program_info']['program_id']) && !empty($props['program_info']['program_title'])) {
                EmpowerCastNetwork::removeOption('program_id', $props['program_info']['program_id']);
                EmpowerCastNetwork::removeOption('program_title', $props['program_info']['program_title']);
            }
            return $props;
        }
        return false;
    ...
    static function getNetworkOwnerInformation($apiUrl, $creds, $networkInfo, $needDirectAPI)
    {
        ...
        {
            $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/account/information/?cache=' . md5( rand(0, 999) . time()) ;
     ...
     * @param $apiUrl string url to Podops api
    ...
    static function refreshTokenToGetCredentials($apiUrl, $createUrl, $tokenInfo)
    {
        ...
        $creds['post'] = $post;
        $props = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true, $clientAuth);
    ...
    static function getFeedFromLink($apiUrl, $creds, $networkInfo, $feedUrl)
    {
        ...
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/applicant/findshow?feedUrl=' . $feedUrl . '&cache=' . md5( rand(0, 999) . time()) ;
        $props = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);
        $requestUrl = $apiUrl.'2/empowercast/network/'.$networkInfo['network_id'].'/lists/';
        $props['list'] = EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);
        return $props;
    }
    ...
    {
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/applicant/submit?listId=' . $add['listIdForApp'] . '&programId=' . $add['programIdForApp'] . '&feedUrl=' . $add['feedUrl'] . '&webName=' . $add['appLabel'];
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);
    }
    ...
    static function updateList($apiUrl, $creds, $networkInfo, $update)
    {
        ...
        $requestUrl = $apiUrl . '2/empowercast/network/' . $networkInfo['network_id'] . '/lists/'.$networkInfo['list_id'].'/edit?title=' . $update['editListTitle'] . '&description=' . $update['editListDescription'];
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);;
    ...
    {
        $creds['post'] = $post;
        $requestUrl = $apiUrl . '2/empowercast/network/token';
        return EmpowercastNetworkDataBus::getCacheOrCallAPI($creds, null, $requestUrl, true);
    }
}
