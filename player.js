/** 
 * jsMediaPlayer 1.7.0 for Podops EmpowerCast
 * 
 * http://www.podopshost.com/powepress/
 *
...
function empowercast_show_embed(id)
{
	if( document.getElementById('empowercast_embed_'+id) ) {
		if( document.getElementById('empowercast_embed_'+id).style.display == 'block' ) {
			document.getElementById('empowercast_embed_'+id).style.display = 'none';
		} else {
			document.getElementById('empowercast_embed_'+id).style.display = 'block';
			document.getElementById('empowercast_embed_'+id +'_t').select();
		}
	}
	return false;
}
...
*/
function empowercast_embed_html5v(id,media_url,width,height,webm_media_url)
{
	if( document.getElementById('empowercast_player_'+id) )
	{
		var poster = '';
		if( document.getElementById('empowercast_player_'+id).getElementsByTagName ) {
			var images = document.getElementById('empowercast_player_'+id).getElementsByTagName('img');
		...
		if( html5 ) {
			...
			document.getElementById('empowercast_player_'+id).innerHTML = '';
			document.getElementById('empowercast_player_'+id).appendChild(v);
			v.play();
	
			if( window.empowercast_resize_player )
				empowercast_resize_player();
...
/**
	...
	@webm_media_url - Alternative WebM media URL
*/
function empowercast_embed_html5a(id,media_url)
{
	if( document.getElementById('empowercast_player_'+id) )
	{
		var poster = '';
		...
		if( html5 ) {
			...
			document.getElementById('empowercast_player_'+id).innerHTML = '';
			document.getElementById('empowercast_player_'+id).appendChild(a);
			a.play();
			return false; // stop the default link from proceeding
		}
	}
...
/**
	EmpowerCast on page load, make sure IE 9+/Edge use a custom flag for playback
*/
function empowercast_onload() {
	var x = document.getElementsByTagName("audio");
	for(i in x) {
		x[i].addEventListener("play", function(e) {
			var found = this.src.match( /media.(podops|rawvoice).(net|biz|com)\/[a-zA-Z_]{3,30}\/p\//i );
			if( found.length > 0 ) {
				this.pause();
...
if ( window.navigator.userAgent.match( /(MSIE|Edge|Trident)\//i )	!== null && window.addEventListener) {
	window.addEventListener('load', empowercast_onload, false);
}

function empowercast_stp(e) {
	e.preventDefault();
