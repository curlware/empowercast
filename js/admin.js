function empowercast_verifyMedia(el) {
    let feed_slug = el.id.replace("verify-button-", "");
    empowercast_get_media_info(feed_slug);
}
...
jQuery(window).on("load", function(){
    ...
    if (url) {
        tb_show('Verify Podops Account', url + '&KeepThis=true&TB_iframe=true&width=600&height=400&modal=true', false);
...
function empowercast_toggle_lock_section(evt) {
...
function empowercast_toggle_guid_section(evt) {
    if (evt.currentTarget.checked) {
...
function empowercast_openTab(evt, cityName) {
    ...
    if (cityName == 'artwork-' + feed_slug) {
        let el = jQuery("#empowercast_itunes_image_" + feed_slug);
        let el_a = jQuery("#empowercast_image_" + feed_slug);
        jQuery.merge(el, el_a);
        if (el.length > 0) {
            interval = setInterval(function () {
                empowercast_insertArtIntoPreview(el[0]);
            }, 1000);
        }
    }
...
function empowercast_displaySideNav(el) {
...
function empowercast_changeExplicitSwitch(el) {
    let id = el.id;
...
function empowercast_toggleMetamarksSettings(el) {
    var feed_slug;
...
function empowercast_showHideMediaDetails(el) {
...
function empowercast_showHideAppleAdvanced(el) {
    let feed_slug;
    let show_det;
    let new_id;
...
function empowercast_changeMediaFile(evt, el) {
    evt.preventDefault();
    ...
    let show_input = jQuery("#empowercast_url_show_" + feed_slug);
    ...
    let podops_info = jQuery("#ep-box-podops-service-" + feed_slug);
    podops_info.removeAttr("style");
    podops_info.attr("style", "display: block");
    let container = jQuery("#pp-media-podops-container-" + feed_slug);
    container.attr("style", "background-color: #f1f4f9; padding: 2ch;");
    if(!verify_interval) {
        verify_interval = setInterval(function() { empowercast_verifyButtonColor(feed_slug); })
    }
}

//save button for edit media link
function empowercast_cancelMediaEdit(el) {
    ...
    let input = jQuery("#pp-url-input-container-" + feed_slug);
    let url_field = jQuery("#empowercast_url_" + feed_slug + " > input");
    let show_input = jQuery("#empowercast_url_show_" + feed_slug);
    let edit_media = jQuery("#edit-media-file-" + feed_slug);
    ...
    podops_info.removeAttr("style");
    podops_info.attr("style", "display: none");
    container.removeAttr("style");
}

//save button for edit media link
function empowercast_saveMediaFile(el) {
    let feed_slug = el.id.replace("save-media-", "");
    empowercast_get_media_info(feed_slug);
    let link = sanitizeHTML(jQuery("#empowercast_url_display_" + feed_slug).val());
    let display_filename = jQuery("#ep-box-filename-" + feed_slug);
    let input = jQuery("#pp-url-input-container-" + feed_slug);
    let show_input = jQuery("#empowercast_url_show_" + feed_slug);
    let edit_media = jQuery("#edit-media-file-" + feed_slug);
...
function empowercast_skipToEpisodeSettings(feed_slug) {
    ...
    let blu_container = jQuery("#pp-media-podops-container-" + feed_slug);
    let connect_info = jQuery("#ep-box-podops-connect-" + feed_slug);
    let connect_info_small = jQuery("#ep-box-min-podops-connect-" + feed_slug);
...
function empowercast_insertArtIntoPreview(el) {
    let feed_slug = el.id.replace("empowercast_itunes_image_", "");
    feed_slug = feed_slug.replace("empowercast_image_", "");
    let art_input = "#empowercast_itunes_image_" + feed_slug;
    let poster_input = "#empowercast_image_" + feed_slug;
...
function empowercast_locationInput(event){
...
function empowercast_epboxPCIToggle(el){
...
function browseChapterImages(event) {
    ...
    g_empowercast_chapters_img_input_id = event.currentTarget.id.replace('image_browser', 'image');
    ...
    window.send_to_editor = function(html)
    {
        ...
        console.log(g_empowercast_chapters_img_input_id);
        console.log(url);
        jQuery('#' + g_empowercast_chapters_img_input_id).val( url ).change();
        tb_remove();
...
function empowercast_chapters_art_preview(el) {
    let img_id = el.id.replace('empowercast_chapters_image_', 'pp-image-chapters-preview-');
    let img_container_id = el.id.replace('empowercast_chapters_image_', 'preview-im-container-');
    let upload_link_container_id = el.id.replace('empowercast_chapters_image_', 'pp-chapters-art-text-');
...
function removeIm(event) {
    ...
    let fileInp = jQuery('#empowercast_chapters_image_' + id_suffix);
...
function empowercast_epboxPCIToggleManual(el){
    let id_array = el.id.split("_");
    if (el.checked) {
...
function setChaptersCheckboxes(id, feed_slug){
    let noChapters = jQuery('#empowercast_chapters_none_' + feed_slug);
    let manualChapters = jQuery('#empowercast_chapters_manual_' + feed_slug);
    let uploadChapters = jQuery('#empowercast_pci_chapters_' + feed_slug);
    let uploadChaptersInput = jQuery('#empowercast_pci_chapters_container_' + feed_slug);
    let chapterBuilderContainer = jQuery('#' + feed_slug + '-chapter-builder-container');

    switch(id){
        case 'empowercast_chapters_none_' + feed_slug:
        ...
        case 'empowercast_chapters_manual_' + feed_slug:
        ...
        case 'empowercast_pci_chapters_' + feed_slug:
...
function setTranscriptCheckboxes(id, feed_slug){
    let noTranscript = jQuery('#empowercast_transcript_none_' + feed_slug);
    let generateTranscript = jQuery('#empowercast_transcript_generate_' + feed_slug);
    let uploadTranscript = jQuery('#empowercast_pci_transcript_' + feed_slug);
    let uploadTranscriptInput = jQuery('#empowercast_pci_transcript_container_' + feed_slug);
    ...
    let generateTranscriptLanguageContainer = jQuery('#empowercast_generate_transcript_container_' + feed_slug);

    switch(id){
        case 'empowercast_transcript_none_' + feed_slug:
        ...
        case 'empowercast_transcript_generate_' + feed_slug:
        ...
        case 'empowercast_pci_transcript_' + feed_slug:
