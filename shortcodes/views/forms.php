...
if (!empty($_POST)) {
    ...
    $post = false;
    $requestUrl = '/2/empowercast/network/' . $props['empowercast_network']['network_id'] . '/applicant/findshow?feedUrl='.$feedUrl ;
    $results = $GLOBALS['ppn_object']->requestAPI($requestUrl, true, $post);
    if (isset($results['program_id'])) {
        $requestUrl = '/2/empowercast/network/' . $props['empowercast_network']['network_id'] . '/applicant/submit';
        ...
        $error = isset($results['alert']) ?
            $results['alert'] : "Show could not be found in Podops directory. Please double check your URL or contact Podops support.";
    }
}
?>
<link href="<?php echo EmpowerCastNetwork::empowercast_network_plugin_url(). "css/style.css";?>" rel="stylesheet">
