<link rel="stylesheet" href="<?php echo EmpowerCastNetwork::empowercast_network_plugin_url(); ?>css/style.css" type="text/css"/>
<link href="<?php echo EmpowerCastNetwork::empowercast_network_plugin_url(). "css/blueprint.css";?>" rel="stylesheet">
<link href="<?php echo empowercast_get_root_url(). "css/subscribe-widget.css";?>" rel="stylesheet">
...
if($props['style'] == 'older-full') {?>
    <div class="ppn-container" id="ppn-program">
        ...
        <?php for ($i = 0; ($i < $props['limit'] && $i < count($props['episodes'])); $i++) {
            ...
            $player = apply_filters('empowercast_player', '', esc_url($props['episodes'][$i]['media_url']), array() );
         ?>
...
<?php }elseif($props['style'] == 'playlist') {?>
    <div class="ppn-grid-header" bp="grid">
        ...
        foreach ($props['episodes'] as $episode) {
            do_action('wp_empowercast_player_scripts');
            $player = apply_filters('empowercast_player', '', esc_url($episode['media_url']), array() );
            ?>
...
<?php } elseif($props['style'] == 'full') {
    ...
    <div class="ppn-grid-header" bp="grid">
        ...
        <div class="ppn-program-sub-btns" bp="12">
            <?php echo empowercast_do_subscribe_sidebar_widget(
                    [
                        ...
                        'subscribe_feature_podops_sidebar'=>false,
                        'podops_url'=>esc_url($props['subscribe_podops']), //For future use
