...
require_once(dirname(__FILE__).'/empowercastadmin-auth.class.php');

class EmpowerCastNetwork
{
    private $display;
    private $apiBus;
    private $parent_slug;
    ...
    function __construct($parent_slug)
    {
        ...
        $this->init();
        $this->apiBus = new EmpowercastNetworkDataBus();
    ...
    function init()
    {
        ...
        if (is_admin() && isset($_GET['page']) && $_GET['page'] == 'empowercast-network') {
            $key = $_GET['key'];
            ...
            switch ($key) {
                ...
                case 'applications':
                    ...
                    break;
                default:
                    include(dirname(__FILE__) . '/empowercast-network.php');
	...
	function getAccessToken()
	{
		...
		$creds = get_option('empowercast_network_creds');
		...
		if( !empty($creds['refresh_token']) && !empty($creds['client_id']) && !empty($creds['client_secret']) ) {
			...
			if( !empty($resultTokens['access_token']) && !empty($resultTokens['expires_in']) ) {
				empowercast_save_settings( array('access_token'=>$resultTokens['access_token'], 'access_expires'=>( time() + $resultTokens['expires_in'] - 10 ) ), 'empowercast_network_creds');
    ...
    function requestAPI($requestUrl, $auth = false, $post = false)
    {
		...
		if( $response === false ) {
            empowercast_page_message_add_error( __('Error: ' . $auth->getLastError(), 'empowercast') );
        }
		return $response;
    }
    ...
    {
        if (is_admin()) {
            wp_register_style('empowercast-network', $this::empowercast_network_plugin_url() . 'css/style.css');
    ...
    static function empowercast_network_plugin_url()
    {
        $local_path = __FILE__;
    ...
    public function network_plugin_setup_menu(){
        //$parent_slug, __('EmpowerCast Network', 'empowercast'), __('EmpowerCast Network', 'empowercast'), EMPOWERCAST_CAPABILITY_EDIT_PAGES, 'network-plugin', 'empowercast_admin_page_network_plugin');
    ...
    {
        $originalMap = get_option('empowercast_network_map');
        $map = $originalMap;
        $pageCreated = false;
        $target = null;
        $postID = null;
        ...
        if (!$pageCreated){
            ...
            $page['post_title'] = __(htmlspecialchars($_POST['pageTitle']), 'empowercast');
            $postID = wp_insert_post($page);
            if ($postID != 0){
                $map[$target] = $postID;
                if($originalMap === null) {
                    add_option('empowercast_network_map', $map);
                } else{
                    update_option('empowercast_network_map', $map);
                }
            }
        }
        return $postID;
    ...
    {
        $option = get_option('empowercast_network_map');
        $postID = 0;
        if (isset($_POST['pageAction']) && $_POST['pageAction'] == 'unlink'){
            ...
            update_option ('empowercast_network_map', $option);
        } else if (isset($_POST['pageAction']) && $_POST['pageAction'] == 'clearSiteCache') {
            $network_id = get_option('empowercast_network_id');
            if ($network_id) {
    ...
    private function handleCodeReturn($apiUrl, $createUrl, &$creds)
    {
		if( empty($_GET['state']) || empty($_GET['code']) ) {
			empowercast_page_message_add_error( __('An error occurred linking your account. Missing parameters.', 'empowercast-network') );
			return false;
		}
		
		$tempClient = get_option('empowercast_network_temp_client');
		if( $_GET['state'] != $tempClient['state'] ) {
			empowercast_page_message_add_error( __('An error occurred linking your account. State does not match.', 'empowercast-network') );
			return false;
		} 
		...
		if( $resultClient === false || empty($resultClient['client_id']) || empty($resultClient['client_secret']) ) {
			if( !empty($resultTokens['error_description']) )
				...
				empowercast_page_message_add_error( __('Error issuing client:','empowercast-network') .' '.$auth->GetLastError() . $auth->getDebugInfo() );
			return false;
		}
		...
		empowercast_save_settings( $props, 'empowercast_network_creds');
		
		empowercast_page_message_add_notice( __('Account linked successfully.', 'empowercast-network') );
		return;
    }

    private function checkSignin($apiUrl, $createUrl, &$creds)
    {
        $option = get_option('empowercast_network_creds');
		$accessToken = $this->getAccessToken();
	...
	public function action_admin_init()
    {
        ...
        if (isset($_POST['unlinkAccount'])){
            delete_option ('empowercast_network_creds');
        }
				...
				case 'link-account': {
					...
					if (isset($_POST['signinRequest'])) {
						...
						if( $result !== false && !empty($result['temp_client_id']) && !empty($result['temp_client_secret']) ) {
							...
							update_option('empowercast_network_temp_client', array('temp_client_id' => $result['temp_client_id'], 'temp_client_secret' =>$result['temp_client_secret'], 'state'=>$state ));
						...
						if( !empty($result['error_description']) )
							empowercast_page_message_add_error( $result['error_description'] );
						else if( !empty($result['error']) )
							empowercast_page_message_add_error( $result['error'] );
						else
							empowercast_page_message_add_error( __('Error creating temporary client:','empowercast-network') .' '.$auth->GetLastError() );
					}
				}; break;
				case 'set-network-id': {
					$networkId = $_POST['networkId'];
					$requestUrl = '/2/empowercast/network/' . $networkId;
					...
					if( !empty($props['network_id']) ) {
						update_option('empowercast_network_id', $networkId);
						if( !empty($props['network_title']) )
							update_option('empowercast_network_title', $props['network_title']);
					} else {
						//delete_option('empowercast_network_id');
						//delete_option('empowercast_network_title');
					}
		...
		//echo "EmpowerCast Network ID: $networkId <br />";
...
//            $_POST['linkPage'] = 'Nothing here';
            switch ($status) {
                case 'List Networks':
                    ...
                    if (isset($_POST['linkNetwork']) && $_POST['linkNetwork'] == 'unlink') {
                        delete_option('empowercast_network');
                    }
                    break;
    ...
    {
        $result = get_option ('empowercast_network');
        unset ($result[$key]);
        update_option('empowercast_network', $result);
    }
    ...
    {
		if( function_exists('empowercast_page_message_print') )
        ...
        if ($alert) { //If there is alert, print it out
            ...
            <div class="alert<?php if (!$danger) echo '-success'; ?>">
                <p class="alertMessage"><?php echo __($alert, 'empowercast-network'); ?></p>
        ...
        ?>
        <link href="<?php echo empowercast_get_root_url() . "css/admin.css"; ?>" rel="stylesheet">
        <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
        <script src="<?php echo $this->empowercast_network_plugin_url() . "js/admin.js"; ?>"></script>
...
<?php
    ...
    function checkUpdateProgram()
    {
        ...
        if (time() >= $timeExecute){
            $network = get_option ('empowercast_network');
			if( empty($networkId) )
				return;
            $apiArray = empowercast_get_api_array();
            $apiUrl = $apiArray[0];
            $post = false; // array('grant_type'=>'client_credentials', 'access_token'=>$accessToken );
            $requestUrl = $apiUrl.'2/empowercast/network/'.$networkId.'/update?since='.($timeExecute - 24*60*60);
            $programUpdate = $this->requestAPI($requestUrl);
            for ($i = 0 ; $i < count ($programUpdate); ++$i) {
                EmpowerCastNetwork::updateMeta($programUpdate[$i]);
            }
        }
    }
}
